...
Job 的 `revision: u64` 只在契约改变时递增。暂停、Stop 和其他使当前调用失效的操作直接撤销该调用的提交资格。CallId 永不复用，Stop 会撤销所有活跃调用及路由，因此第一版不再额外维护全局 generation。

第一版不自动重试业务操作，CallId 同时作为工具适配器可用的本次操作标识；不保留与 CallId 永久一一对应的 EffectId。重复完成事件按 CallId 去重。Unknown 写入只能由宿主确证，不能自动换一个 CallId 重发。
...
Delegate 校验整个列表、一次检查容量、原子创建全部子工作，并投递 ChildCreated；不足则一个都不建，向父工作记录明确的容量结果，不能偷偷建立一部分。它返回后父 Job Ready，不自动等待所有子工作。

Read 是只读 Kernel 记录查询，结果即时投递并重新唤醒调用者；不调用业务工具或额外模型。Jobs 按 JobId 翻页，parent 为空列活跃根工作，否则列直属子工作；Job 读一个已知工作卡片。目录页最多 16 条；存在后页时 `next_job` 是本页最后一个 ID，调用者把它传给下一次 `ReadQuery::Jobs.after`。Record 按不可变正文的 UTF-8 字节偏移分段，Kernel 限制长度并维护字符边界，返回 `next_offset`。读取回执保存来源及范围，不再复制大正文。

Worker 可读自己记录、直属子工作的公开卡片/成果及显式共享记录；拥有子工作不自动开放它的私有工具历史。协调者可读同一宿主会话的公开目录、报告与允许的证据。权限由调用者、owner 和已接受的共享/投递记录导出，不能仅凭猜中 Seq 获取正文。收到信封只开放它明确交付的内容，不自动展开全部引用；查询仍返回有界预览。该边界不声称多租户隔离。
...
Kernel 用 `BTreeMap<Seq, Arc<Record>>` 保存正文。RecordBody 包含原始输入、委派说明、工作笔记、工具事实、报告、发布成果、Outcome、询问、回答、投递信封及导入记忆。Origin 由 Kernel 填写用户 Input 或 Job/Call/版本来源，模型不能提供它。
...
CoordinateInput 只含本次路由原话、约束、活跃根工作第一页和该路由已取得的相关查询结果。JobCard 从 Spec、真实状态及当前有效 Report 派生，不保存另一份可变目录。默认页同时受 16 条和 `context_bytes` 约束；`next_job` 给出继续读取的游标。历史或子工作通过 Read 查询。
...
4. 展开 Delivery 和 ReadResult 明确引用的正文，并按 `(Seq, offset)` 去重。
...
seed 只接受合法创建路径允许复用的 Completed Job：Worker 可选择自己的已完成子工作；用户路由可选择当前请求允许的会话记忆。仅能看见 JobCard 不授予完整 Context 读取权。Kernel 从来源 checkpoint 或 Outcome 取得有界 summary 和 evidence，给新 Job 加入 `ImportedMemory { source_job, source_revision, summary, record_refs }`。新 Job 的 checkpoint 为空，`read_through` 从零开始；旧 checkpoint 的 through 不会泄漏进新 Job。

首次输入只必须容纳来源身份和有界记忆摘要；record_refs 是可按需读取的背景证据，不把全部旧历史变成新 Job 必须展开的未读消息。源 Job 没有 checkpoint 时，以 Outcome 摘要和 evidence 开始。保留来源的工作事实，不复制旧的未决问题、active_call、暂停标记、路由权或用户交付义务。当前进程不清理源记录；未来引入 retention 时必须为缺失引用定义明确的不可用语义。

第一版继续结构化模型协议，不依赖 provider 私有 replay。若将来启用原生历史，只保存真实 assistant 输出与真实处理回执；`submit_work.operation` 里的业务工具不能伪造成模型直接发出的原生 tool_call。
...
结构错误或越权引用使整份提案拒绝，进入明确调用失败路径，没有局部更新。本调用确实收到过、但现已超时/取消/结算的询问，其回答是过时 no-op，只留审计，其余有效内容继续提交；未知或不属于本调用的关联 ID 才是结构错误。容量不足属于可观察等待或拒绝接收，不等于结构错误。Delegate 容量不足返回接收失败回执，让父 Worker 重新决定；Tool、对外 Reply、Finish 的暂时门禁可保存 PendingStep。
...
`try_step(Finish)` 依次检查：当前契约/暂停有效；本次 answers 提交后没有未决询问；必要投递已读；没有本 Job 未完成的工具；所有拥有子工作已终态且 Outcome 已读；**整个拥有子树没有未决外部写**；用户输入解释门禁允许对应交付。
...
解释输入时对已完成工作的跟进，其 owner 是当前 Routing；由此沿拥有关系获得调查写限制，Outcome 仅内部返回。Worker 不能把普通 User Job 自行改成 Routing Job。新用户输入取代旧批次时，撤销旧路由及其调查树的资格；已发生工具效果仍记账。
...
Pause 清除子树有效调用及待提交动作，保留真实等待原因，并请求取消正在执行的调用；Commit 等待随候选一起清除并变为 Ready。Resume 只清除指定 local_paused。原本 Ready 的 Job 恢复后可调度；工具已结束或定时已到的等待也可重新检查。Tick 消费已到期限，即使暂停也不继续把过期时刻作为 next_deadline，避免空转；暂停门禁保持。任何内核材料都不能隐式 Resume。
...
| 3 | Kernel 创建、提案提交、Delegate、成果等待、Finish | 父能看到 child ID；失败子工作不能被当成成功；子树 Unknown 阻止成功交付 |
| 4 | Runtime 端口、取消、时间、轮转与观察 | 取消资格后仍计实际容量；控制不等模型；慢观察者不阻塞执行 |
| 5 | Inquiry、Routing、seed 与 Compact | 内部回答不被 Finish 门禁扣住；旧批次不复活；旧 checkpoint 不污染新 Job 水位 |
...
- Runtime 受控 futures 与暂停时钟：实际并发容量、超时、迟到外部效果、shutdown 与观察者掉队。

当前 30 个库测试用小而确定的事件序列覆盖输入幂等、原子 Delegate、Worker Context 隔离、父子完成门禁、seed、Pause/Resume、Stop、迟到模型与工具事实、clarification reply、全局问题串行、Routing 调查树、Inquiry 终态、依赖 revision、constraints 失效、Outcome 去重、目录分页和固定前缀压缩。Runtime 测试覆盖外部写取消后的真实结果与同步 panic 的槽释放；模型测试覆盖精确结构协议。

纯 Rust 测试证明状态与协议，不证明模型能正确拆分和总结。实现稳定后再用真实模型比较原话要求保留、错误完成、无谓委派、重复调查、上下文大小和完成耗时；这些是效果评估，不增加到 Kernel 状态机。
