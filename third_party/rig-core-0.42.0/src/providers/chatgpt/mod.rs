...
const RECONNECT_REQUIRED: &str =
    "ChatGPT authentication was rejected; reconnect the subscription and try again";
const PROVIDER_FAILURE: &str = "ChatGPT provider request failed";
...
/// ChatGPT-only transport decorator for one rejected streaming handshake.
///
/// `GenericEventSource` owns the HTTP handshake lazily, so retry belongs below
/// the event parser. Converting the request body to cloneable bytes lets this
/// decorator rebuild the same request exactly once with a refreshed token.
#[derive(Clone)]
struct ChatGPTStreamingClient<H> {
    inner: H,
    auth: auth::Authenticator,
    rejected_access_token: String,
}

impl<H> HttpClientExt for ChatGPTStreamingClient<H>
where
    H: HttpClientExt + Clone + 'static,
{
    fn send<T, U>(
        ...
        Output = http_client::Result<http::Response<http_client::LazyBody<U>>>,
    > + WasmCompatSend
    + 'static
    where
    ...
    fn send_multipart<U>(
        &self,
        req: http::Request<http_client::MultipartForm>,
    ) -> impl std::future::Future<
        Output = http_client::Result<http::Response<http_client::LazyBody<U>>>,
    > + WasmCompatSend
    + 'static
        ...
        T: Into<Bytes> + WasmCompatSend,
    {
        ...
        let rejected_access_token = self.rejected_access_token.clone();

        async move {
            let first =
                rebuild_streaming_request(&method, &uri, version, &headers, body.clone(), None)?;
            match inner.send_streaming(first).await {
                Err(error) if is_unauthorized_http_error(&error) => {
                    let context = authenticator
                        .refresh_after_rejection(&rejected_access_token)
                        .await
                        .map_err(|_| {
                            http_client::Error::InvalidStatusCode(http::StatusCode::UNAUTHORIZED)
                        })?;
                    let retry = rebuild_streaming_request(
                        &method,
                        &uri,
                        version,
                        &headers,
                    ...
                    match inner.send_streaming(retry).await {
                        Err(error) if is_authentication_http_error(&error) => {
                            authenticator
                                .invalidate_after_rejection(&context.access_token)
                                .await
                                .map_err(|_| {
                                    http_client::Error::InvalidStatusCode(
...
fn rebuild_streaming_request(
    method: &http::Method,
    uri: &http::Uri,
    version: http::Version,
    headers: &http::HeaderMap,
    body: Bytes,
    refreshed: Option<&auth::AuthContext>,
) -> http_client::Result<http::Request<Bytes>> {
    ...
    if let Some(context) = refreshed {
        request.headers_mut().insert(
            http::header::AUTHORIZATION,
            http::HeaderValue::from_str(&format!("Bearer {}", context.access_token))?,
        );
        if let Some(account_id) = &context.account_id {
            request.headers_mut().insert(
                http::HeaderName::from_static("chatgpt-account-id"),
                http::HeaderValue::from_str(account_id)?,
            );
        } else {
    ...
    H: Clone + Default + Debug + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    async fn send_completion(
        ...
        let text = loop {
            ...
            let response = match self.client.send(req).await {
                ...
                Err(error) if is_unauthorized_http_error(&error) && !refreshed => {
                    auth = self
                        .client
                        .ext()
                        .auth
                        .refresh_after_rejection(&auth.access_token)
                        .await
                        .map_err(|_| authentication_error())?;
                    refreshed = true;
                    continue;
                ...
                Err(error) if is_authentication_http_error(&error) => {
                    if refreshed {
                        self.client
                            .ext()
                            .auth
                            .invalidate_after_rejection(&auth.access_token)
                            .await
                            .map_err(|_| authentication_error())?;
                    }
            ...
            if !status.is_success() {
                return Err(sanitize_chatgpt_error(CompletionError::from_http_response(
                    status, text,
                )));
            }
            break text;
        ...
        let raw_response = responses_api::streaming::parse_sse_completion_body(&text, "ChatGPT")
            .map_err(sanitize_chatgpt_error)?;
    ...
    async fn normalized_completion(
        ...
        let response = match raw_response.clone().normalize(PROVIDER_NAME) {
            ...
            Err(error) => return Err(sanitize_chatgpt_error(error)),
        };
    ...
    H: Clone + Default + Debug + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    > {
        ...
        let client = ChatGPTStreamingClient {
            inner: self.client.clone(),
            auth: self.client.ext().auth.clone(),
            rejected_access_token: auth.access_token,
        };
        let event_source = crate::http_client::sse::GenericEventSource::new(client, req)
            .allow_missing_content_type();

        let stream = responses_api::streaming::raw_stream_from_event_source(event_source, span);

        // ChatGPT backend/SSE errors can contain provider response bodies.
        // This provider's public stream boundary intentionally exposes a
        // stable reconnect-required message instead of those raw diagnostics.
        Ok(Box::pin(
            stream.map(|item| item.map_err(sanitize_chatgpt_error)),
        ))
    }
}
...
fn authentication_error() -> CompletionError {
    CompletionError::ProviderError(RECONNECT_REQUIRED.into())
}

fn sanitize_chatgpt_error(error: CompletionError) -> CompletionError {
    match error {
        CompletionError::HttpError(ref transport) if is_authentication_http_error(transport) => {
            authentication_error()
        }
        // Responses SSE provider error envelopes contain the provider's raw
        // body. They are never needed for request control after this boundary.
        CompletionError::ProviderResponse(_) => {
            CompletionError::ProviderError(PROVIDER_FAILURE.into())
        }
        // Preserve all other structured errors. In particular, Rig classifies
        // status-less SSE transport failures (timeout, DNS, reset) as
        // `ProviderError`, so flattening that variant would misreport ordinary
        // network failures as a reconnect requirement.
        other => other,
    }
}

fn is_unauthorized_http_error(error: &http_client::Error) -> bool {
    error.non_success_status() == Some(http::StatusCode::UNAUTHORIZED)
}

fn is_authentication_http_error(error: &http_client::Error) -> bool {
    matches!(
...
mod tests {
    ...
    struct RejectedStreamingClient {
        requests: std::sync::Arc<std::sync::Mutex<Vec<http::HeaderMap>>>,
    }

    impl RejectedStreamingClient {
        fn requests(&self) -> Vec<http::HeaderMap> {
    ...
    impl HttpClientExt for RejectedStreamingClient {
        fn send<T, U>(
            &self,
            _req: http::Request<T>,
        ) -> impl std::future::Future<
        ...
        {
            match self.requests.lock() {
                Ok(mut requests) => requests.push(req.headers().clone()),
                Err(poisoned) => poisoned.into_inner().push(req.headers().clone()),
            }
            std::future::ready(Err(http_client::Error::InvalidStatusCodeWithMessage(
                http::StatusCode::UNAUTHORIZED,
                "sentinel-secret-stream-rejection".into(),
            )))
        }
    }
...
data: [DONE]"#;
    ...
    #[tokio::test]
    async fn completion_redacts_authentication_body_but_preserves_other_failures() {
        use crate::client::CompletionClient;
        use crate::completion::CompletionModel;
        use crate::test_utils::RecordingHttpClient;

        let secret = "sentinel-secret-expired-token";
        let http_client =
            RecordingHttpClient::with_error_response(http::StatusCode::UNAUTHORIZED, secret);
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
        ...
        let error = model
            ...
            .await
            .expect_err("unauthorized completion should fail");
    ...
    #[tokio::test]
    async fn rejected_refreshed_unary_generation_is_invalidated_after_one_retry() {
        use crate::client::CompletionClient;
        use crate::completion::CompletionModel;
        ...
        let transport = SequencedHttpClient::new([
            MockHttpResponse::error(
                http::StatusCode::UNAUTHORIZED,
                "sentinel-secret-initial-rejection",
            ),
            MockHttpResponse::error(
                http::StatusCode::UNAUTHORIZED,
                "sentinel-secret-retry-rejection",
            ),
        ]);
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
                access_token: "builder-token".to_owned(),
                account_id: None,
            })
            .http_client(transport.clone())
            .build()
            .expect("client should build");
        let (authenticator, control) = auth::Authenticator::for_rejection_test(
            auth::AuthContext {
                access_token: "initial-token".to_owned(),
                account_id: Some("initial-account".to_owned()),
            },
            auth::AuthContext {
                access_token: "refreshed-token".to_owned(),
                account_id: Some("refreshed-account".to_owned()),
            },
        );
        let mut extension = client.ext().clone();
        extension.auth = authenticator;
        let client = client.with_ext(extension);
        let model = client.completion_model(GPT_5_4);

        let error = model
            .completion(model.completion_request("hello").build())
            .await
            .expect_err("the retried token should also be rejected");
        let rendered = format!("{error:?}: {error}");
        assert!(rendered.contains("reconnect"), "{rendered}");
        assert!(!rendered.contains("sentinel-secret"), "{rendered}");
        assert_eq!(control.refreshes(), 1, "a request refreshes at most once");
        assert_eq!(control.invalidated_tokens(), ["refreshed-token".to_owned()]);

        let requests = transport.requests();
        assert_eq!(requests.len(), 2, "the request is retried at most once");
        assert_eq!(
            header_value(&requests[0].headers, "authorization").as_deref(),
        ...
        assert_eq!(
            header_value(&requests[1].headers, "chatgpt-account-id").as_deref(),
            Some("refreshed-account")
        );
    }

    #[tokio::test]
    async fn rejected_refreshed_lazy_stream_generation_is_invalidated_after_one_retry() {
        use crate::client::CompletionClient;
        use crate::completion::CompletionModel;

        let transport = RejectedStreamingClient::default();
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
                access_token: "builder-token".to_owned(),
                account_id: None,
        ...
        let mut stream = model
            ...
            .await
            .expect("streaming handshake is lazy");
        let error = stream
            .next()
            .await
            .expect("lazy handshake should yield an error")
            .expect_err("the retried token should also be rejected");
        let rendered = format!("{error:?}: {error}");
        assert!(rendered.contains("reconnect"), "{rendered}");
        assert!(!rendered.contains("sentinel-secret"), "{rendered}");
        assert_eq!(control.refreshes(), 1, "a stream refreshes at most once");
        assert_eq!(control.invalidated_tokens(), ["refreshed-token".to_owned()]);

        let requests = transport.requests();
        assert_eq!(requests.len(), 2, "the handshake is retried at most once");
        assert_eq!(
        ...
        assert_eq!(
            header_value(&requests[1], "chatgpt-account-id").as_deref(),
            Some("refreshed-account")
        );
    }

    #[tokio::test]
    async fn streaming_handshake_and_in_band_errors_are_redacted() {
        use crate::client::CompletionClient;
        use crate::completion::CompletionModel;
        use crate::test_utils::{HttpErrorStreamingClient, SequencedStreamingHttpClient};

        let handshake_secret = "sentinel-secret-streaming-401";
        let http_client =
            HttpErrorStreamingClient::new(http::StatusCode::UNAUTHORIZED, handshake_secret);
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
                access_token: "test-token".to_string(),
                account_id: Some("account-id".to_string()),
        ...
        let mut stream = model
            .stream(model.completion_request("hello").build())
            .await
            .expect("stream is opened lazily");
        let error = stream
            .next()
            .await
            .expect("stream error item")
            .expect_err("401 handshake must fail");
        assert!(error.to_string().contains("reconnect"), "{error}");
        assert!(!error.to_string().contains(handshake_secret), "{error}");
        assert!(
            !format!("{error:?}").contains(handshake_secret),
            "{error:?}"
        );

        let in_band_secret = "sentinel-secret-sse-envelope";
        let body = format!(
            "data: {{\"type\":\"error\",\"error\":{{\"message\":\"{in_band_secret}\",\"code\":\"server_error\",\"type\":\"server_error\"}}}}\n\n"
        );
        let http_client = SequencedStreamingHttpClient::new(vec![Ok(Bytes::from(body))]);
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
                access_token: "test-token".to_string(),
                account_id: Some("account-id".to_string()),
        ...
        assert!(
            error.to_string().contains("provider request failed"),
            "{error}"
        );
        assert!(!error.to_string().contains(in_band_secret), "{error}");
        assert!(!format!("{error:?}").contains(in_band_secret), "{error:?}");

        let http_client =
            HttpErrorStreamingClient::new(http::StatusCode::TOO_MANY_REQUESTS, "rate limited");
        let client = crate::providers::chatgpt::Client::builder()
            .api_key(ChatGPTAuth::AccessToken {
                access_token: "test-token".to_string(),
                account_id: Some("account-id".to_string()),
        ...
        let error = stream
            .next()
            .await
            .expect("rate-limit error item")
            .expect_err("rate-limited stream must fail");
        assert_eq!(
            error.provider_response_status(),
            Some(http::StatusCode::TOO_MANY_REQUESTS)
        );
