...
This directory contains Bone's platform sidecars for the fixed local semantic-search model.

Release CI builds a CPU-only CrispEmbed `v0.15.0` / ggml sidecar for each supported
...
The runtime dynamically loads the target's CrispEmbed library plus its three adjacent
ggml libraries. No daemon, port, model provider, or user configuration is involved.
