...
**Source:** [`@frelion/bone-tui`](https://github.com/frelion/bone/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@frelion/bone-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `BONE_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@frelion/bone-tui";
...
Import from `@frelion/bone-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@frelion/bone-tui";
```
...
```typescript
import { matchesKey, Key } from "@frelion/bone-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@frelion/bone-tui";
...
```typescript
import { getMarkdownTheme } from "@frelion/bone-coding-agent";
import { Markdown } from "@frelion/bone-tui";
...
Set `BONE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
BONE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@frelion/bone-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@frelion/bone-coding-agent";
import { DynamicBorder } from "@frelion/bone-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@frelion/bone-tui";
...
```typescript
import { BorderedLoader } from "@frelion/bone-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@frelion/bone-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@frelion/bone-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@frelion/bone-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@frelion/bone-coding-agent";
import { matchesKey, truncateToWidth } from "@frelion/bone-tui";
