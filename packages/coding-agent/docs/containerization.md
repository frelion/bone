...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.bone/agent/extensions/gondolin
cd ~/.bone/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.bone/agent/extensions/gondolin
```
...
RUN npm install -g --ignore-scripts @frelion/bone-coding-agent
...
```bash
docker build -t pi-sandbox -f Dockerfile.bone .

docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.bone/agent \
  pi-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.bone/agent` if you want container-local settings and sessions. Mounting your host `~/.bone/agent` exposes host auth and session files to the container.
