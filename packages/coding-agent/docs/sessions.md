...
Sessions auto-save to `~/.bone/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
For the on-disk schema, see [Session Format](session-format.md).
