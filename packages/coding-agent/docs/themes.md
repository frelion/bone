> Bone can create themes. Ask it to build one for your setup.
...
Bone loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.bone/agent/themes/*.json`
- Project: `.bone/themes/*.json` (only after the project is trusted)
...
On first run, Bone detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.bone/agent/themes
vim ~/.bone/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/frelion/bone/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Bone reloads it automatically for immediate visual feedback.
...
Bone uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Bone falls back to the nearest approximation.
