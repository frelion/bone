...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".bone", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".bone", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".bone", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".bone"), { recursive: true });
			writeFileSync(join(cwd, ".bone", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".bone"), { recursive: true, force: true });
