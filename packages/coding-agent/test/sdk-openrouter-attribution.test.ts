...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.BONE_TELEMETRY;
		delete process.env.BONE_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.BONE_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/frelion/bone");
		expect(headers?.["X-OpenRouter-Title"]).toBe("Bone");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("Bone");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("bone");
	});
