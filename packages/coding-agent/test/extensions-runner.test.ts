...
describe.skip("ExtensionRunner filesystem compatibility tests", () => {
	...
	describe("message and entry renderers", () => {
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(pi) {
					pi.registerMessageView("my-type", () => undefined);
				}
			`;
			...
			const renderer = runner.getMessageView("my-type");
			expect(renderer).toBeDefined();

			const missing = runner.getMessageView("not-exists");
			expect(missing).toBeUndefined();
		});
		...
		it("gets entry renderer by type", async () => {
			const extCode = `
				export default function(pi) {
					pi.registerEntryView("my-entry", () => undefined);
				}
			`;
			...
			expect(runner.getEntryView("my-entry")).toBeDefined();
			expect(runner.getEntryView("not-exists")).toBeUndefined();
		});
	});
