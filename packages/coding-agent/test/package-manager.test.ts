...
Content`,
		...
		it("should resolve project paths relative to .bone", async () => {
			const extDir = join(tempDir, ".bone", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".bone"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".bone", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".bone", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".bone", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".bone", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".bone", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		...
		it("should use the project .bone dir as baseDir for project .bone skills", async () => {
			const projectBaseDir = join(tempDir, ".bone");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".bone", "agent");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .bone auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".bone\n");

			const skillDir = join(tempDir, ".bone", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.bone\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".bone", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("source parsing", () => {
		...
		it("should recognize github URLs without git: prefix", async () => {
			...
			vi.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand").mockRejectedValue(
				new Error("expected clone failure"),
			);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pi settings base", () => {
			...
			const rel = relative(join(tempDir, ".bone"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".bone"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".bone", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".bone", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".bone", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".bone", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".bone", "npm", "node_modules", "example");
