...
describe("OpenTUIInteractiveMode", () => {
	...
	test("opens /name as an inline prompt and restores composer focus", async () => {
		const active = createRuntime();
		const setSessionName = vi.fn();
		Object.assign(active.session, { setSessionName });
		const host = new FakeHost(active.runtime);
		const renderer = await createNativeTestRenderer({ width: 100, height: 28 });
		const mode = new OpenTUIInteractiveMode(host, {
			createRenderer: async () => renderer,
		...
		await mode.init();
		await renderer.input.typeText("/name");
		renderer.input.pressEnter();
		const frame = await renderer.waitForFrameText("Conversation name");
		expect(frame).toContain("Conversation name");
		expect((mode as unknown as { overlayManager: { active: unknown } }).overlayManager.active).toBeFalsy();
		await renderer.input.typeText("Release work");
		renderer.input.pressEnter();
		await mode.idle();
		await settle(renderer);
