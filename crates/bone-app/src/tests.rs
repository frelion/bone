...
#[tokio::test]
async fn config_changes_are_desired_until_the_runtime_is_replaced() {
    let (_temporary, app, workspace) = configured_app().await;
    ...
    wait_for_input(&session, receipt.input).await;
    let first = app.resolved_config(session.id()).await.unwrap();
    let running = first.running.unwrap();
    ...
    assert_eq!(changed.running, Some(running));
    assert_eq!(changed.desired.unwrap().worker.selection.model, "new-model");
    app.shutdown().await.unwrap();
}
