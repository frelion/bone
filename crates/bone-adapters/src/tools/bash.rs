...
#[cfg(unix)]
fn kill_process_group(process_group: Option<rustix::process::Pid>) -> io::Result<()> {
...
mod tests {
    ...
    #[cfg(unix)]
    async fn wait_for_pid(path: &Path) -> rustix::process::Pid {
    ...
    #[cfg(unix)]
    async fn wait_until_gone(pid: rustix::process::Pid) -> bool {
        let deadline = tokio::time::Instant::now() + Duration::from_secs(2);
        loop {
