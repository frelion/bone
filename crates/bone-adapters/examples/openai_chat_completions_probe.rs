        ...
        if !matches!(
            ...
            [tool_call]
                if tool_call.name() == "inspect_path"
                    && tool_call.arguments()["path"] == "/tmp/bone-adapters-probe"
        ) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "tool probe expected exactly one inspect_path call for /tmp/bone-adapters-probe",
            )
            .into());
        }
    }
...
Usage:
  cargo run -p bone-adapters --example openai_chat_completions_probe -- [text|tool]
