use bone_llm::{
    ConfigError, Protocol, protocol::openai_responses, testing::openai_responses_endpoint,
};
use rig_core::{providers::openai as rig_openai, test_utils::RecordingHttpClient};
...
fn separates_endpoint_protocol_and_model_identity() {
    ...
    let client_b = rig_openai::Client::builder()
        ...
        .build()
        .expect("test client should build");
    let endpoint_a = openai_responses_endpoint("gateway-a", client_a).unwrap();
    let endpoint_b = openai_responses_endpoint("gateway-b", client_b).unwrap();
