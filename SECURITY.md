...
This document describes Bone's security model and its boundaries.

Bone is a coding agent that runs locally within the security boundary
...
Bone treats the local user account and files writable by that account as inside
the same trust boundary as the Bone process itself. If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Bone configuration, they can generally influence Bone or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Bone grants that write access or crosses
an operating-system privilege boundary.

Bone relies on users installing trustworthy extensions and loading trustworthy
skills and only to use Bone within trusted repositories. This is because files
...
If you believe you found a security vulnerability in Bone or another package in
...
- Local code execution or sandboxing behavior (Bone intentionally does not have a sandbox)
- Behavior of Bone extensions or skills installed by the user
...
- Public internet exposure of a Bone installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.bone`,
  `~/.bone/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Bone itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against Bone.
...
For example, a report showing that malicious contents written to a trusted Bone
configuration file cause Bone to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Bone. For exposed-secret reports,
