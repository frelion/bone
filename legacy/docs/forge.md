...
Plan Mode can use `forge_context`, `forge_query`, `forge_audit`, and `forge_watch`. Execution mode additionally provides `forge_issue`, `forge_milestone`, `forge_change`, `forge_wiki`, `forge_pipeline`, `forge_release`, `forge_variable`, and `forge_transition`.
...
CI/CD variables use a separate `forge_variable` mutation tool with `create`, `update`, and `delete` actions. Variable lists expose metadata but omit values; reading a value requires an explicit single-key `forge_query` call. Values never appear in collapsed summaries, mutation receipts, or the mutation journal. GitLab environment scopes and variable flags are supported; GitHub Actions variables reject GitLab-only fields explicitly.
