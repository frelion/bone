...
- Agent stage explanations are normal assistant prose in the transcript. They remain visible, use the same typography as other assistant text, and never receive an activity highlight.
- A stage normally contains several consecutive Actions. Inspection, design, editing, targeted testing, and routine fixes are Action-level transitions and do not require another prose update; a new stage explanation appears only for a genuinely new phase or a material result that changes the approach.
- An Action is a user-understandable semantic objective, not an individual tool call. One Action can span multiple ModelTurns and contain multiple ToolCalls.
- Live Actions are always declared explicitly by the Agent protocol. Bone never infers a semantic Action from an ordinary ToolCall. A stage explanation must declare its first Action with `set_action` in the same response; later Actions in that stage use an exclusive `set_action` response without prose, and ordinary tools run only while an Action is active.
- Invalid protocol responses are rejected atomically before any ordinary tool starts. The rejection is returned privately to the model for correction and is not transcript content. Three consecutive corrected attempts are allowed; a fourth violation fails the Exchange explicitly instead of inferring intent or executing tools.
- The active Action occupies the activity position and owns the rolling highlight. Completed Actions remain as static, collapsed summaries.
- Clicking an Action summary expands or collapses its ToolCall summaries. Each ToolCall independently has exactly two detail states, `collapsed` and `full`; clicking its summary toggles complete parameters and results, and `Ctrl+O` applies the same hierarchy globally.
- Failed ToolCalls use the semantic error color but remain collapsed until the user explicitly opens them. Failure never auto-expands tool details.
- `Working` is a temporary rolling-highlight waiting state while an Exchange has no active semantic Action, including while the model is producing or correcting an Action declaration. It is never persisted as transcript content and disappears with the final answer.
