...
import { getDefaultForgeAgentDir } from "./config-path.ts";
...
import type { ForgeService, ForgeToolContext, ForgeToolName } from "./service-contracts.ts";
	...
	if (toolName === "forge_variable") {
		if (provider === "github") {
			const unsupported = [
				...
				"raw",
				"description",
			].find((key) => body[key] !== undefined);
			if (unsupported) {
				throw new ForgeError("unsupported_capability", `GitHub Actions variables do not support ${unsupported}`);
			}
			return defined({ value: body.value });
		}
		return defined({
			key: id,
			value: body.value,
			variable_type: body.variableType,
			environment_scope: body.environmentScope,
...
class DefaultForgeService implements ForgeService {
	...
	constructor(options: CreateForgeServiceOptions) {
		...
		this.agentDir = options.agentDir ?? getDefaultForgeAgentDir();
	...
	private async query(
		...
		if (operation === "get" && resource !== "variable" && input.id === undefined) {
			throw new ForgeError("validation_failed", "get requires id");
		}
		if (operation === "get" && resource === "variable" && input.key === undefined) {
			throw new ForgeError("validation_failed", "variable get requires key");
		}
		...
		if (operation === "get") {
			if (resource === "variable") {
				const key = stringInput(input.key, "key");
				const environmentScope = typeof input.environmentScope === "string" ? input.environmentScope : undefined;
				const value =
					resolved.adapter instanceof GitLabAdapter
						? await resolved.adapter.getVariable(resolved.repository.projectPath, key, environmentScope, signal)
						: await resolved.adapter.getVariable(resolved.repository.projectPath, key, signal);
				return projectForgeDetail(resource, value);
			}
			return projectForgeDetail(resource, await this.fetchQueryDetail(resolved, resource, input.id, signal));
		}
		if (resource === "variable" && operation === "get_many") {
			throw new ForgeError("validation_failed", "CI/CD variables do not support get_many; get one explicit key");
		}
		if (operation === "get_many" && ids) {
		...
		if (resolved.adapter instanceof GitLabAdapter) {
			switch (resource) {
				...
				case "variable":
					return projectForgePage(resource, await resolved.adapter.listVariables(project, query, signal));
			}
		}
									...
									: capability === "pipeline"
										...
										: capability === "job"
											...
											: capability === "release"
												...
												: capability === "variable"
													? `/api/v4/projects/${project}/variables?per_page=1`
													: undefined
				: capability === "issue"
						...
						: capability === "pull_request"
							...
							: capability === "actions"
								...
								: capability === "release"
									...
									: capability === "variable"
										? `/repos/${project}/actions/variables?per_page=1`
										: undefined;
		if (!path) {
	...
	private async mutate(
		...
		const project = resolved.repository.projectPath;
		const inputPayload = inputBody(input);
		const rawBody =
			toolName === "forge_variable" && input.environmentScope !== undefined
				? { ...inputPayload, environmentScope: input.environmentScope }
				: inputPayload;
		let id = input.id;
	...
	private mutateGitLab(
		...
		if (toolName === "forge_variable") {
			const key = stringInput(id, "variableKey");
			const scope = typeof body.environment_scope === "string" ? body.environment_scope : undefined;
			if (action === "create") return adapter.createVariable(project, body, signal);
			if (action === "update") return adapter.updateVariable(project, key, body, scope, signal);
			if (action === "delete") return adapter.deleteVariable(project, key, scope, signal).then(() => ({ key }));
		}
	...
	private async mutateGitHub(
		...
		if (toolName === "forge_variable") {
			...
			if ((action === "create" || action === "update") && value === undefined) {
				throw new ForgeError("validation_failed", `GitHub variable ${action} requires value`);
			}
			if (action === "create") return adapter.createVariable(project, key, value ?? "", signal);
			if (action === "update") return adapter.updateVariable(project, key, value ?? "", signal);
			if (action === "delete") return adapter.deleteVariable(project, key, signal).then(() => ({ name: key }));
		}
		if (toolName === "forge_wiki") {
			throw new ForgeError("unsupported_capability", "GitHub Wiki does not have a supported REST API", {
				capability: "wiki",
