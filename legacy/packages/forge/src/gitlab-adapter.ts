...
const CAPABILITY_PATHS: ReadonlyArray<{ id: GitLabCapabilityId; path: (project: string) => string }> = [
	...
	{ id: "variable", path: (project) => `/api/v4/projects/${project}/variables?per_page=1` },
];
...
function requireVariable(value: unknown, operation: string): Record<string, unknown> {
	if (typeof value !== "object" || value === null || typeof (value as { key?: unknown }).key !== "string") {
		throw new ForgeError("invalid_remote_response", `GitLab ${operation} response is missing variable key`, {
			operation,
		});
	}
	return value as Record<string, unknown>;
...
export class GitLabAdapter {
	...
	async listVariables(
		...
		const path = `/api/v4/projects/${encodedProject(project)}/variables`;
		const response = await this.client.request<unknown>("GET", path, { query: { per_page: 10, ...query }, signal });
		if (!Array.isArray(response.data)) {
			throw new ForgeError("invalid_remote_response", "GitLab variable response is not a list", { operation: path });
		}
		const items = response.data.map((item) => requireVariable(item, path));
		const nextCursor = headerValue(response.headers["x-next-page"]);
		return { items, nextCursor: nextCursor || undefined, hasMore: Boolean(nextCursor) };
	}
	...
	async getVariable(
		...
		const response = await this.client.request<unknown>(
			"GET",
			`/api/v4/projects/${encodedProject(project)}/variables/${encodeURIComponent(key)}`,
			{ query: { "filter[environment_scope]": environmentScope }, signal },
		);
		return requireVariable(response.data, "get variable");
	}
	...
	async createVariable(
		...
		const response = await this.client.request<unknown>(
			"POST",
			`/api/v4/projects/${encodedProject(project)}/variables`,
			{
				body,
				signal,
				sensitiveValues: typeof body.value === "string" ? [body.value] : [],
			},
		);
		return requireVariable(response.data, "create variable");
	}
	...
	async updateVariable(
		...
		const response = await this.client.request<unknown>(
			...
			{
				body,
				query: { "filter[environment_scope]": environmentScope },
				signal,
				sensitiveValues: typeof body.value === "string" ? [body.value] : [],
			},
		);
		return requireVariable(response.data, "update variable");
	}

	async deleteVariable(project: Project, key: string, environmentScope?: string, signal?: AbortSignal): Promise<void> {
