...
function requireVariable(value: unknown, operation: string): Record<string, unknown> {
	if (typeof value !== "object" || value === null || typeof (value as { name?: unknown }).name !== "string") {
		throw new ForgeError("invalid_remote_response", `GitHub ${operation} response is missing variable name`, {
			provider: "github",
			operation,
		});
	}
...
export class GitHubAdapter {
	...
	async getCapabilities(repository: string, signal?: AbortSignal): Promise<ForgeCapability[]> {
		...
		const probes = [
			...
			["variable", `/repos/${repo}/actions/variables?per_page=1`],
		] as const;
	...
	async listVariables(
		...
		const response = await this.client.request<unknown>(
			"GET",
			`/repos/${encodedRepository(repository)}/actions/variables`,
			{
		...
		const container =
			typeof response.data === "object" && response.data !== null
				? (response.data as { variables?: unknown }).variables
				: undefined;
		if (!Array.isArray(container)) {
			throw new ForgeError("invalid_remote_response", "GitHub variable response is not a list", {
				provider: "github",
			});
		}
		const cursor = nextPage(headerValue(response.headers.link));
		return {
			items: container.map((item) => requireVariable(item, "list variables")),
			nextCursor: cursor,
			hasMore: cursor !== undefined,
		};
	}

	async getVariable(repository: string, key: string, signal?: AbortSignal): Promise<Record<string, unknown>> {
		const response = await this.client.request<unknown>(
			"GET",
			`/repos/${encodedRepository(repository)}/actions/variables/${encodeURIComponent(key)}`,
			{ signal },
		);
	...
	async createVariable(
		...
		await this.client.request("POST", `/repos/${encodedRepository(repository)}/actions/variables`, {
			body: { name: key, value },
			signal,
	...
	async deleteVariable(repository: string, key: string, signal?: AbortSignal): Promise<void> {
		await this.client.request(
			"DELETE",
			`/repos/${encodedRepository(repository)}/actions/variables/${encodeURIComponent(key)}`,
			{ signal },
