...
export type ForgeQueryResource =
	| "issue"
	| "milestone"
	| "change"
	| "wiki"
...
export function projectForgeResource(
	...
	const projected = compactRecord({
		...
		key:
			text(entry.slug, 256) ??
			text(entry.tag_name, 256) ??
			(resource === "variable" ? text(entry.key ?? entry.name, 256) : undefined),
		...
		value: resource === "variable" && mode === "detail" ? text(entry.value, MAX_FORGE_DETAIL_BODY_BYTES) : undefined,
		variableType: resource === "variable" ? text(entry.variable_type ?? entry.variableType, 64) : undefined,
		environmentScope:
			resource === "variable" ? text(entry.environment_scope ?? entry.environmentScope, 256) : undefined,
		protected: resource === "variable" ? boolean(entry.protected) : undefined,
		masked: resource === "variable" ? boolean(entry.masked) : undefined,
		hidden: resource === "variable" ? boolean(entry.hidden) : undefined,
		raw: resource === "variable" ? boolean(entry.raw) : undefined,
		description: resource === "variable" ? text(entry.description) : undefined,
	});
...
export function projectForgeMutation(
	...
	const keyedResource = resource === "wiki" || resource === "release" || resource === "variable";
	return compactRecord({
		...
		key:
			text(entry.slug, 256) ??
			text(entry.tag_name, 256) ??
			text(entry.key ?? entry.name, 256) ??
			(keyedResource ? text(reference, 256) : undefined),
		state: text(entry.state, 128) ?? text(entry.status, 128),
		webUrl: text(entry.web_url, MAX_URL_TEXT) ?? text(entry.html_url, MAX_URL_TEXT),
	});
}
