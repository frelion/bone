...
describe("GitHubAdapter", () => {
	...
	it("lists, reads, creates, updates, and deletes Actions variables", async () => {
		const { adapter: github, mock } = adapter();
		mock
			.intercept({ method: "GET", path: "/repos/acme/widget/actions/variables?per_page=2&page=2" })
			.reply(200, { total_count: 1, variables: [{ name: "DEPLOY_ENV", value: "production" }] });
		mock
			.intercept({ method: "GET", path: "/repos/acme/widget/actions/variables/DEPLOY_ENV" })
			.reply(200, { name: "DEPLOY_ENV", value: "production" });
		mock
			.intercept({
				method: "POST",
				path: "/repos/acme/widget/actions/variables",
				body: JSON.stringify({ name: "DEPLOY_ENV", value: "staging" }),
			})
			.reply(201, "");
		mock
			.intercept({
				method: "PATCH",
				path: "/repos/acme/widget/actions/variables/DEPLOY_ENV",
				body: JSON.stringify({ name: "DEPLOY_ENV", value: "production" }),
			})
			.reply(204, "");
		mock.intercept({ method: "DELETE", path: "/repos/acme/widget/actions/variables/DEPLOY_ENV" }).reply(204, "");

		await expect(github.listVariables("acme/widget", { per_page: 2, page: 2 })).resolves.toMatchObject({
			items: [{ name: "DEPLOY_ENV", value: "production" }],
		});
		await expect(github.getVariable("acme/widget", "DEPLOY_ENV")).resolves.toMatchObject({ value: "production" });
		await expect(github.createVariable("acme/widget", "DEPLOY_ENV", "staging")).resolves.toEqual({
			name: "DEPLOY_ENV",
		});
		await expect(github.updateVariable("acme/widget", "DEPLOY_ENV", "production")).resolves.toEqual({
			name: "DEPLOY_ENV",
		});
		await expect(github.deleteVariable("acme/widget", "DEPLOY_ENV")).resolves.toBeUndefined();
	});
});
