...
import { createForgeService } from "../src/service.ts";
import type { ForgeToolContext } from "../src/service-contracts.ts";
...
describe("DefaultForgeService", () => {
	...
	it("resolves public GitHub context and negotiates capabilities", async () => {
		...
		for (const path of [
			...
			"/repos/acme/widget/actions/variables?per_page=1",
		]) {
	...
	it("lists variable metadata, reads one value, and writes a value without persisting it", async () => {
		vi.stubEnv("GITLAB_TOKEN", "service-secret");
		const api = dispatcher.get("https://gitlab.com");
		api.intercept({ method: "GET", path: "/api/v4/projects/acme%2Fwidget/variables?per_page=1" }).reply(200, []);
		api.intercept({ method: "GET", path: "/api/v4/projects/acme%2Fwidget/variables?per_page=10" }).reply(200, [
			{ key: "DEPLOY_TOKEN", value: "list-secret", masked: true, protected: true },
		]);
		api.intercept({ method: "GET", path: "/api/v4/projects/acme%2Fwidget/variables/DEPLOY_TOKEN" }).reply(200, {
			key: "DEPLOY_TOKEN",
		...
		api.intercept({
			method: "PUT",
			path: "/api/v4/projects/acme%2Fwidget/variables/DEPLOY_TOKEN",
			body: JSON.stringify({ key: "DEPLOY_TOKEN", value: "rotated-secret", protected: true, masked: true }),
		}).reply(200, { key: "DEPLOY_TOKEN", value: "rotated-secret", masked: true, protected: true });
		const { service, context } = harness(true);

		const list = await service.execute(
			"forge_query",
			{ remote: "https://gitlab.com/acme/widget.git", operation: "list", resource: "variable" },
			undefined,
			context,
		);
		expect(list).toMatchObject({
			resource: "variable",
			items: [{ key: "DEPLOY_TOKEN", masked: true, protected: true }],
		});
		expect(JSON.stringify(list)).not.toContain("list-secret");
		...
		).resolves.toMatchObject({ item: { key: "DEPLOY_TOKEN", value: "current-secret" } });
		...
		const receipt = await service.execute(
			"forge_variable",
			{
				...
				action: "update",
				id: "DEPLOY_TOKEN",
				requestId: "rotate-deploy-token",
				input: { value: "rotated-secret", masked: true, protected: true },
			},
			undefined,
			context,
		);
		...
		const journal = readFileSync(join(context.agentDir!, "forge-mutations.json"), "utf8");
		expect(journal).not.toContain("rotated-secret");
	});
