...
import { parseGitRemote } from "../src/context-resolver.ts";
import { ForgeError } from "../src/errors.ts";
import { GitLabAdapter, parseGitLabVersion } from "../src/gitlab-adapter.ts";
import { ForgeHttpClient } from "../src/http-client.ts";
import { assertPublicNetworkHostname, isPrivateNetworkAddress } from "../src/network-security.ts";
...
describe("GitLab Forge adapter", () => {
	...
	it("maps capability probes without disabling the entire integration", async () => {
		...
		pool.intercept({ path: "/api/v4/projects/5/variables?per_page=1", method: "GET" }).reply(200, []);
...
describe("Git remote context", () => {
	...
	it("lists, reads, updates, and deletes project CI variables without leaking write values", async () => {
		const { adapter, mock } = createAdapter();
		const pool = mock.get("http://gitlab.internal.example");
		pool
			.intercept({ method: "GET", path: "/api/v4/projects/acme%2Fwidget/variables?per_page=2&page=3" })
			.reply(200, [{ key: "DEPLOY_ENV", value: "production", protected: true }]);
		pool
			.intercept({
				method: "GET",
				path: "/api/v4/projects/acme%2Fwidget/variables/DEPLOY_ENV?filter%5Benvironment_scope%5D=prod",
			})
			.reply(200, { key: "DEPLOY_ENV", value: "production", environment_scope: "prod" });
		pool
			.intercept({
				method: "PUT",
				path: "/api/v4/projects/acme%2Fwidget/variables/DEPLOY_ENV?filter%5Benvironment_scope%5D=prod",
				body: JSON.stringify({ key: "DEPLOY_ENV", value: "rotated", environment_scope: "prod" }),
			})
			.reply(500, { message: "rejected rotated" });
		pool.intercept({ method: "DELETE", path: "/api/v4/projects/acme%2Fwidget/variables/DEPLOY_ENV" }).reply(204, "");

		await expect(adapter.listVariables("acme/widget", { per_page: 2, page: 3 })).resolves.toMatchObject({
			items: [{ key: "DEPLOY_ENV", value: "production", protected: true }],
		});
		await expect(adapter.getVariable("acme/widget", "DEPLOY_ENV", "prod")).resolves.toMatchObject({
			key: "DEPLOY_ENV",
			value: "production",
		});
		const updateError = await adapter
			.updateVariable(
				"acme/widget",
				"DEPLOY_ENV",
				{ key: "DEPLOY_ENV", value: "rotated", environment_scope: "prod" },
				"prod",
			)
			.catch((error: unknown) => error);
		expect(String(updateError)).toContain("[REDACTED]");
		expect(String(updateError)).not.toContain("rotated");
		await expect(adapter.deleteVariable("acme/widget", "DEPLOY_ENV")).resolves.toBeUndefined();
	});
