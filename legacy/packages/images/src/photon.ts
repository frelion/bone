 ...
 * 1. Bun source/package execution (development, bun run build)
 ...
 * 2. Resolve photon_rs_bg.wasm from the embedded standalone runtime or next to
 *    the executable for legacy/package builds
 */
...
function getFallbackWasmPaths(): string[] {
	...
	return [
		...(process.env.BONE_RUNTIME_DIR ? [path.join(process.env.BONE_RUNTIME_DIR, WASM_FILENAME)] : []),
