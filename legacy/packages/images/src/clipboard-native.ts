...
export function loadClipboardNative(
	requires: readonly ClipboardRequire[] = [
		moduleRequire,
		...(process.env.BONE_RUNTIME_DIR
			? [createRequire(pathToFileURL(join(process.env.BONE_RUNTIME_DIR, "package.json")).href)]
			: []),
		executableDirRequire,
	],
