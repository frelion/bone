...
import type { AgentSessionEvent } from "@frelion/bone-coding-agent";
import type { RpcCommand, RpcExtensionUIRequest, RpcExtensionUIResponse, RpcResponse } from "@frelion/bone-protocol";
...
export class RpcProcessInstance {
	...
	private getSpawnCommand(): { command: string; args: string[] } {
		if (isBunBinary) {
			return {
				command: join(dirname(process.execPath), process.platform === "win32" ? "bone.exe" : "bone"),
				args: ["--mode", "rpc"],
			};
		}
		return {
			command: process.execPath,
			args: [require.resolve("@frelion/bone-coding-agent/rpc-entry")],
		};
	}
