import { type AgentMessage, uuidv7 } from "@frelion/bone-agent-core";
...
export interface SessionMessageEntry extends SessionEntryBase {
	...
	/** Stable product-level exchange containing this message. */
	exchangeId?: string;
	/** Pi model decision turn containing this message. */
	modelTurnId?: string;
	/** How a persisted user message entered its exchange. */
	delivery?: "prompt" | "steer" | "follow_up";
	/** Whether an assistant response continues, completes, or was rejected from its exchange. */
	responseDisposition?: "continuation" | "final" | "rejected";
}

export type SessionMessageMetadata = Pick<
	SessionMessageEntry,
	"exchangeId" | "modelTurnId" | "delivery" | "responseDisposition"
>;
...
export function parseSessionEntryLine(line: string): FileEntry | null {
	...
	try {
		...
		return isValidFileEntryMetadata(entry) ? entry : null;
	} catch {
		return null;
	}
}
...
function isCurrentSessionHeader(value: unknown): value is SessionHeader {
	return (
		isRecord(value) &&
		value.type === "session" &&
...
function isSessionEntryBase(value: Record<string, unknown>): boolean {
	return (
...
function isImageContent(value: unknown): boolean {
	return (
		isRecord(value) && value.type === "image" && typeof value.data === "string" && isNonEmptyString(value.mimeType)
	);
}
...
function isPersistedMessage(value: unknown): value is AgentMessage {
	if (!isRecord(value) || !isNonEmptyString(value.role) || !isFiniteNumber(value.timestamp)) return false;
	switch (value.role) {
		case "user":
		...
		case "assistant":
			return (
				...
				value.content.every((part) => isTextContent(part) || isThinkingContent(part) || isToolCallContent(part)) &&
		...
		case "custom":
			return (
				isNonEmptyString(value.customType) && isUserContent(value.content) && typeof value.display === "boolean"
			);
		case "bashExecution":
			return (
...
function hasValidExchangeMetadata(value: Record<string, unknown>): boolean {
	if (
		(value.exchangeId !== undefined && !isNonEmptyString(value.exchangeId)) ||
		(value.modelTurnId !== undefined && !isNonEmptyString(value.modelTurnId))
	) {
		return false;
	}
	if (
	...
	return (
		value.responseDisposition === undefined ||
		value.responseDisposition === "continuation" ||
		value.responseDisposition === "final" ||
		value.responseDisposition === "rejected"
	);
}

function isValidFileEntryMetadata(value: unknown): value is FileEntry {
	if (!isRecord(value) || typeof value.type !== "string") return false;
	if (value.type === "session") return isCurrentSessionHeader(value);
	if (!isSessionEntryBase(value)) return false;
	switch (value.type) {
		case "message":
			return (
				isPersistedMessage(value.message) &&
				hasValidExchangeMetadata(value) &&
				!(
		...
		case "thinking_level_change":
			return isNonEmptyString(value.thinkingLevel);
		case "model_change":
			return isNonEmptyString(value.provider) && isNonEmptyString(value.modelId);
		case "compaction":
			return (
		...
		case "branch_summary":
			return isNonEmptyString(value.fromId) && typeof value.summary === "string";
		case "custom":
			return isNonEmptyString(value.customType);
		case "label":
			return isNonEmptyString(value.targetId) && (value.label === undefined || typeof value.label === "string");
		case "session_info":
			...
			return (
				...
				(value.toolsBeforePlanMode === undefined ||
					(Array.isArray(value.toolsBeforePlanMode) &&
						value.toolsBeforePlanMode.every((tool) => isNonEmptyString(tool))))
			);
		case "plan_proposal":
			return (
				isRecord(value.proposal) &&
				isNonEmptyString(value.proposal.id) &&
				typeof value.proposal.version === "number" &&
				typeof value.proposal.content === "string" &&
				isNonEmptyString(value.proposal.createdAt) &&
				isNonEmptyString(value.proposal.sourceMessageId)
			);
		case "plan_decision":
			return (
				isNonEmptyString(value.proposalId) &&
				typeof value.version === "number" &&
				(value.decision === "approved" || value.decision === "revision_requested" || value.decision === "cancelled")
			);
		case "question_asked":
			return isRecord(value.request) && isNonEmptyString(value.request.id) && Array.isArray(value.request.questions);
		case "question_answered":
			return (
				isNonEmptyString(value.requestId) &&
				Array.isArray(value.answers) &&
				(value.overallNotes === undefined || typeof value.overallNotes === "string")
			);
		case "question_cancelled":
			return (
				...
				(value.reason === "user" ||
					value.reason === "abort" ||
					value.reason === "client_disconnect" ||
					value.reason === "no_ui")
			);
		default:
			return false;
...
export function loadEntriesFromFile(filePath: string): FileEntry[] {
	...
	try {
		...
		else if (pending.trim()) invalid = true;
	} finally {
		closeSync(fd);
	}

	if (invalid || entries.length === 0 || !isCurrentSessionHeader(entries[0])) return [];
	return entries;
}
...
function readSessionHeader(filePath: string): SessionHeader | null {
	try {
		...
		return isCurrentSessionHeader(header) ? header : null;
	} catch {
		return null;
	}
}
...
export class SessionManager {
	...
	appendMessage(message: Message | CustomMessage | BashExecutionMessage, metadata?: SessionMessageMetadata): string {
		return this.appendMessageWithPersistence(message, metadata).id;
	}

	/** Append a message and return every entry that became physically durable in this write. */
	appendMessageWithPersistence(
		...
		const entry: SessionMessageEntry = {
			...
			message,
			...(metadata?.exchangeId && { exchangeId: metadata.exchangeId }),
			...(metadata?.modelTurnId && { modelTurnId: metadata.modelTurnId }),
			...(metadata?.delivery && { delivery: metadata.delivery }),
			...(metadata?.responseDisposition && { responseDisposition: metadata.responseDisposition }),
		};
	...
	appendCustomMessageEntry<T = unknown>(
		...
		return this.appendCustomMessageEntryWithPersistence(customType, content, display, details).id;
	}

	/** Append a context-participating custom message and report entries made durable by the write. */
	appendCustomMessageEntryWithPersistence<T = unknown>(
		customType: string,
		content: string | (TextContent | ImageContent)[],
		display: boolean,
		details?: T,
		metadata?: SessionMessageMetadata,
	): { id: string; persistedEntries: SessionEntry[] } {
