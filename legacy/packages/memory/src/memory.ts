...
import { getTextContentPhase, type TextContent } from "@frelion/bone-ai";
import {
...
function textFromCustomEntry(entry: CustomMessageEntry): string {
...
function isFinalAssistantMessage(
	...
	disposition: "continuation" | "final" | "rejected" | undefined,
): boolean {
	if (!isMessageWithContent(message) || message.role !== "assistant") return false;
	return disposition === "final";
}

function finalAssistantText(message: AgentMessage): string {
	if (!isMessageWithContent(message) || message.role !== "assistant") return "";
	if (typeof message.content === "string") return message.content;
	const finalBlocks = message.content.filter(
		(part): part is TextContent => part.type === "text" && getTextContentPhase(part) === "final_answer",
	);
	return (
		finalBlocks.length > 0 ? finalBlocks : message.content.filter((part): part is TextContent => part.type === "text")
	)
		.map((part) => part.text)
		.join("\n");
}
...
export class MemoryRuntime {
	...
	async recordPersistedEntries(
		...
		for (const entry of entries) {
			...
			const exchangeInput =
				entry.type === "custom_message" && entry.exchangeId && entry.delivery
					? { text: textFromCustomEntry(entry), exchangeId: entry.exchangeId }
					: entry.type === "message" &&
							entry.exchangeId &&
							isMessageWithContent(entry.message) &&
							entry.message.role === "user"
						? { text: textFromMessage(entry.message), exchangeId: entry.exchangeId }
						: undefined;
			if (exchangeInput) {
				const userText = normalizeSearchPreview(exchangeInput.text, 2_000);
				if (!userText) continue;
				const pending = this.pendingBySession.get(session.path) ?? [];
				const existing = pending.find((item) => item.exchangeId === exchangeInput.exchangeId);
				const exchangeState: PendingExchange = existing ?? {
					id: stableHash(`${session.path}:exchange:${entry.id}`),
					sourceEntryId: entry.id,
					userText,
					createdAt: Date.parse(entry.timestamp),
					exchangeId: exchangeInput.exchangeId,
				};
				if (existing) exchangeState.userText = `${existing.userText}\n\n${userText}`;
				else pending.push(exchangeState);
				this.pendingBySession.set(session.path, pending);
				const exchange = createExchangeItem({
					sessionPath: session.path,
					sessionId: session.id,
					sourceEntryId: exchangeState.sourceEntryId,
					userText: exchangeState.userText,
					createdAt: exchangeState.createdAt,
				});
			...
			if (
				...
				entry.exchangeId &&
				isFinalAssistantMessage(entry.message, entry.responseDisposition)
			) {
				const pending = this.pendingBySession
					.get(session.path)
					?.find((item) => item.exchangeId === entry.exchangeId);
				if (pending) await this.completePendingExchange(session, pending, entry.message);
			}
		}
	}

	private async completePendingExchange(
		session: { path: string; id: string },
		pending: PendingExchange,
		assistant: AgentMessage,
	): Promise<void> {
		const assistantText = normalizeSearchPreview(finalAssistantText(assistant), 4_000);
...
export async function extractMemoryItems(session: SessionInfo): Promise<MemoryItem[]> {
	...
	const pendingByExchangeId = new Map<string, PendingExchange>();
	for (const line of raw.split(/\r?\n/)) {
		...
		if (entry.type === "custom_message" && entry.exchangeId && entry.delivery) {
			const text = normalizeSearchPreview(textFromCustomEntry(entry), 4_000);
			if (!text) continue;
			const existing = pendingByExchangeId.get(entry.exchangeId);
			if (existing) existing.userText = `${existing.userText}\n\n${text}`;
			else {
				pendingByExchangeId.set(entry.exchangeId, {
					id: stableHash(`${session.path}:exchange:${entry.id}`),
					sourceEntryId: entry.id,
					userText: text,
					createdAt: Date.parse(entry.timestamp),
		...
		if (entry.type !== "message" || !isMessageWithContent(entry.message)) continue;
		const text = normalizeSearchPreview(
			entry.message.role === "assistant" ? finalAssistantText(entry.message) : textFromMessage(entry.message),
			4_000,
		);
		if (!text) continue;
		if (entry.message.role === "user" && entry.exchangeId) {
			const existing = pendingByExchangeId.get(entry.exchangeId);
			if (existing) {
				existing.userText = `${existing.userText}\n\n${text}`;
				continue;
			}
			const pending = {
				id: stableHash(`${session.path}:exchange:${entry.id}`),
				...
				createdAt: Date.parse(entry.timestamp),
				exchangeId: entry.exchangeId,
			} satisfies PendingExchange;
			pendingByExchangeId.set(entry.exchangeId, pending);
			continue;
		}
		if (!isFinalAssistantMessage(entry.message, entry.responseDisposition)) continue;
		if (!entry.exchangeId) continue;
		const user = pendingByExchangeId.get(entry.exchangeId);
		if (!user) continue;
		const exchange = createExchangeItem({
			sessionPath: session.path,
			sessionId,
		...
		pendingByExchangeId.delete(entry.exchangeId);
	}
	for (const user of pendingByExchangeId.values()) {
		const exchange = createExchangeItem({
			sessionPath: session.path,
			sessionId,
			sourceEntryId: user.sourceEntryId,
