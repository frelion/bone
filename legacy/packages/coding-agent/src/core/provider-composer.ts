...
import { getBuiltinApi } from "@frelion/bone-ai/api/builtins";
...
export function composeModelProvider(
	...
	const streamWith = (
		...
		lazyStream(model, async () => {
			...
			const api = getBuiltinApi(model.api);
			if (!api) throw new Error(`No built-in API implementation for api: ${model.api}`);
			return simple
