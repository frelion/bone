...
import type { ExtensionUIToolSummaryInput } from "../extensions/ui-v2.ts";
...
const variableKey = Type.String({
	minLength: 1,
	maxLength: 256,
	pattern: "^[A-Za-z_][A-Za-z0-9_]*$",
	description: "CI/CD variable key",
});
	...
	Type.Object(
		{
			...
			resource: Type.Literal("variable"),
			...paginationFields,
		},
		closed,
	),
	Type.Object(
		{
			...
			resource: Type.Literal("variable"),
			key: variableKey,
			environmentScope: Type.Optional(nonEmptyString("GitLab environment scope", 256)),
		},
		closed,
	),
	Type.Object(
...
const variableEnvironmentScope = Type.Optional(nonEmptyString("GitLab environment scope", 256));
const variableFields = {
	value: Type.Optional(nonEmptyString("Variable value; treated as sensitive", 64 * 1024)),
	variableType: Type.Optional(Type.Union([Type.Literal("env_var"), Type.Literal("file")])),
	environmentScope: variableEnvironmentScope,
	protected: Type.Optional(Type.Boolean()),
	masked: Type.Optional(Type.Boolean()),
	hidden: Type.Optional(Type.Boolean()),
	raw: Type.Optional(Type.Boolean()),
	description: Type.Optional(shortText("Variable description")),
};
const variableMutationSchema = Type.Union([
	mutationVariant(
		"create",
		{ variableKey },
		Type.Object(
			{ ...variableFields, value: nonEmptyString("Variable value; treated as sensitive", 64 * 1024) },
			closed,
		),
	),
	mutationVariant("update", { variableKey }, Type.Object(variableFields, nonEmptyClosed)),
	mutationVariant("delete", { variableKey, environmentScope: variableEnvironmentScope }),
]);
...
function displayResource(value: unknown, count?: number): string {
	const resource = typeof value === "string" ? value.replaceAll("_", " ") : "resource";
	return count === 1 || resource.endsWith("s") ? resource : `${resource}s`;
}

function displayIdentifier(resource: unknown, identifier: string | undefined): string {
	if (!identifier) return "";
	return resource === "wiki" || resource === "release" || resource === "variable"
		? ` ${identifier}`
		: ` #${identifier}`;
}

const actionSummary: Readonly<Record<string, readonly [running: string, complete: string]>> = {
	...
	play_job: ["starting", "started"],
	retry_job: ["retrying", "retried"],
	cancel_job: ["cancelling", "cancelled"],
	submit_review: ["submitting", "submitted"],
	release: ["releasing", "released"],
};

function forgeToolSummary(toolName: ForgeToolName, input: ExtensionUIToolSummaryInput): string | undefined {
	const args = record(input.args) ?? {};
	const details = record(input.result?.details);
	if (input.phase === "failed") return undefined;
	const pending = input.phase !== "complete";
	const phase = input.phase === "preparing" || input.phase === "ready" ? input.phase : pending ? "running" : undefined;

	if (toolName === "forge_context") {
		const repository = record(details?.repository);
		const project = summaryValue(repository?.projectPath);
		const provider = summaryValue(repository?.provider);
		return project && !pending
			? `Forge context · ${provider ?? "forge"} · ${project}`
			: `Forge context · resolving repository · ${phase ?? "complete"}`;
	}

	if (toolName === "forge_query") {
		const operation = summaryValue(args.operation) ?? "query";
		const resource = summaryValue(args.resource) ?? summaryValue(details?.resource);
		if (!pending) {
			const returned = typeof details?.returned === "number" ? details.returned : undefined;
			if ((operation === "list" || operation === "get_many") && returned !== undefined) {
				const verb = operation === "list" ? "listed" : "loaded";
				return `Forge query · ${verb} ${returned} ${displayResource(resource, returned)}`;
			}
			const item = record(details?.item);
			const identifier = summaryValue(item?.number ?? item?.key ?? item?.id ?? args.id);
			return `Forge query · loaded ${displayResource(resource, 1)}${displayIdentifier(resource, identifier)}`;
		}
		return `Forge query · ${operation} ${displayResource(resource)} · ${phase}`;
	}

	if (toolName === "forge_audit") {
		const workflow = summaryValue(args.workflow)?.replaceAll("_", " ") ?? "workflow";
		if (!pending && typeof details?.compliant === "boolean") {
			return `Forge audit · ${workflow} · ${details.compliant ? "compliant" : "blocked"}`;
		}
		return `Forge audit · ${workflow} · ${phase ?? "complete"}`;
	}

	if (toolName === "forge_watch") {
		const resource = displayResource(args.resource, 1);
		const identifier = summaryValue(args.id);
		const state = summaryValue(details?.state);
		return `Forge watch · ${resource}${identifier ? ` #${identifier}` : ""} · ${state ?? phase ?? "complete"}`;
	}

	const action = summaryValue(args.action ?? args.transition) ?? "update";
	const resultResource = summaryValue(details?.resource);
	const resource = displayResource(resultResource ?? toolName.replace(/^forge_/, ""), 1);
	const identifier = summaryValue(
		details?.number ??
			details?.key ??
			details?.id ??
			args.issueNumber ??
	...
	const verb = actionSummary[action]?.[pending ? 0 : 1] ?? action.replaceAll("_", " ");
	const state = !pending ? summaryValue(details?.state) : undefined;
	return `Forge ${resource} · ${verb}${displayIdentifier(resultResource ?? toolName.replace(/^forge_/, ""), identifier)}${state ? ` · ${state}` : ` · ${phase ?? "complete"}`}`;
}
...
const specs: ForgeToolSpec[] = [
	...
	{
		name: "forge_variable",
		label: "Forge variable",
		description:
			"Create, update, or delete repository CI/CD variables. Values are sensitive and never appear in summaries or mutation receipts.",
		promptSnippet: "Manage repository CI/CD variables",
		parameters: variableMutationSchema,
		contract: writeContract("destructive"),
	},
	{
	...
	return Object.fromEntries(
		specs.map((spec) => {
			const definition: ToolDefinition = {
				...
				renderV2: { summarize: (input) => forgeToolSummary(spec.name, input) },
