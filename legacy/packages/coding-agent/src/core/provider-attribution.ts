import type { Api, Model, ProviderHeaders } from "@frelion/bone-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://github.com/frelion/bone",
			"X-OpenRouter-Title": "Bone",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Bone",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "bone-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "bone" };
}
