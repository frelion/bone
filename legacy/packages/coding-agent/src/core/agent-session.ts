...
import {
	...
	type ExchangeProjectorEvent,
	type NarrativeItem,
	projectExchangeEvent,
} from "./exchange/index.ts";
...
import {
	...
	createSemanticActionToolDefinition,
	SEMANTIC_ACTION_TOOL_NAME,
} from "./tools/index.ts";
...
/** Receives immutable product-level Exchange projections after each projected lifecycle change. */
export type AgentSessionExchangeProjectionListener = (projection: ExchangeProjection) => void;
...
export interface AgentSessionConfig {
	...
	/** Session-scoped child-agent manager owned by this AgentSession. */
	subagentManager?: CodingSubagentManager;
}
...
const MAX_PROTOCOL_CORRECTIONS = 3;
...
export class AgentSession {
	...
	private _exchangeProjectionListeners: AgentSessionExchangeProjectionListener[] = [];
	private _isAgentRunActive = false;
	private _idleWaitPromise: Promise<void> | undefined;
	private _resolveIdleWait: (() => void) | undefined;
	private _pendingModelTurnId: string | undefined;
	private _agentAbortRequested = false;
	private _protocolCorrectionCount = 0;
	private readonly _messageDeliveries = new WeakMap<AgentMessage, NonNullable<SessionMessageMetadata["delivery"]>>();
	private _exchangeProjection: ExchangeProjection;
	...
	private readonly _subagentManager?: CodingSubagentManager;
	private _subagentScopeCloseTail: Promise<void> = Promise.resolve();
	private _subagentShutdownPromise?: Promise<void>;
	...
	constructor(config: AgentSessionConfig) {
		...
		this._exchangeProjection = createExchangeProjection(this.sessionManager.getSessionId());
	...
	private get _activeExchangeId(): string | undefined {
		return this._exchangeProjection.activeExchangeId;
	}

	private get _activeModelTurnId(): string | undefined {
		return (
			this._getActiveExchange()?.modelTurns.find((turn) => turn.status === "running")?.id ?? this._pendingModelTurnId
		);
	}

	private _getActiveExchange() {
		const exchangeId = this._exchangeProjection.activeExchangeId;
		return this._exchangeProjection.exchanges.find((exchange) => exchange.id === exchangeId);
	}
	...
	private _installAgentToolHooks(): void {
		this.agent.beforeAssistantResponse = async ({ assistantMessage }) => {
			const response = classifyAgentProtocolResponse(assistantMessage, {
				hasActiveAction: this._getActiveAction() !== undefined,
				hasPendingSteering: this._steeringMessages.length > 0,
			});
			const { violation } = response;
			if (!violation) {
				this._protocolCorrectionCount = 0;
				return { responseMetadata: response };
			}

			this._protocolCorrectionCount++;
			const terminate = this._protocolCorrectionCount > MAX_PROTOCOL_CORRECTIONS;
			const reason = terminate
				? `${violation.message} The ${MAX_PROTOCOL_CORRECTIONS}-attempt protocol correction limit was exceeded.`
				: violation.message;
			return {
				block: true,
				reason,
				details: createAgentProtocolErrorDetails(
					violation.code,
					this._protocolCorrectionCount,
					MAX_PROTOCOL_CORRECTIONS,
				),
				correctionMessage: createAgentProtocolCorrectionMessage(
					{ ...violation, message: reason },
					this._protocolCorrectionCount,
					MAX_PROTOCOL_CORRECTIONS,
				),
				terminate,
		...
		this.agent.beforeToolCall = async ({ toolCall, args }) => {
			if (toolCall.name === SEMANTIC_ACTION_TOOL_NAME) return undefined;
	...
	private async _emitAgentSettled(): Promise<void> {
		...
		try {
			const exchangeId = this._activeExchangeId;
			if (exchangeId) {
				this._completeActiveAction(this._actionStatusForExchange());
				this._projectExchange({
					type: "exchange_completed",
					exchangeId,
					status: this._exchangeOutcome(),
					at: Date.now(),
				});
				this._queueSubagentExchangeClose(exchangeId);
				await this._subagentScopeCloseTail;
			}
			...
			this._pendingModelTurnId = undefined;
			this._agentAbortRequested = false;
			this._protocolCorrectionCount = 0;
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		const assistantResponse =
			event.type === "message_end" && event.message.role === "assistant"
				? this._getAssistantResponse(event)
				: undefined;
		this._trackExchangeLifecycle(event);
		...
		if (event.type === "message_end" && event.message.role === "assistant") {
			this._completeAssistantNarratives(event.message, assistantResponse!);
		}
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				const result = this.sessionManager.appendCustomMessageEntryWithPersistence(
					event.message.customType,
					event.message.content,
					event.message.display,
					event.message.details,
					this.getCurrentExchangeMetadata(event.message, assistantResponse?.disposition),
				);
				...
				const result = this.sessionManager.appendMessageWithPersistence(
					event.message,
					this.getCurrentExchangeMetadata(event.message, assistantResponse?.disposition),
				);
			...
			if (event.message.role === "assistant") {
				...
				if (
					...
					assistantResponse?.disposition === "final" &&
	...
	private _trackExchangeLifecycle(event: AgentEvent): void {
		if (event.type === "turn_start") {
			this._pendingModelTurnId = randomUUID();
			return;
		}
		if (event.type === "message_update" && event.message.role === "assistant") {
			this._ensureActiveModelTurnStarted();
			this._syncStreamingCommentary(event.message);
			return;
		}
		if (event.type === "tool_execution_start") {
			if (!this._activeExchangeId || !this._activeModelTurnId) return;
			this._ensureActiveModelTurnStarted();
			if (event.toolName === SEMANTIC_ACTION_TOOL_NAME) return;
			const action = this._getActiveAction();
			if (!action) throw new Error(`Agent protocol invariant violated: ${event.toolName} started without an Action`);
			this._projectExchange({
				type: "action_tool_call_started",
				exchangeId: this._activeExchangeId,
				actionId: action.id,
				modelTurnId: this._activeModelTurnId,
		...
		if (event.type === "tool_execution_update") {
			if (!this._activeExchangeId || event.toolName === SEMANTIC_ACTION_TOOL_NAME) return;
			const action = this._getActionForToolCall(event.toolCallId);
			if (!action) return;
			this._projectExchange({
		...
		if (event.type === "tool_execution_end") {
			...
			const action = this._getActionForToolCall(event.toolCallId);
			if (!action) return;
			const status = this._agentAbortRequested ? "cancelled" : event.isError ? "failed" : "completed";
			this._projectExchange({
		...
		if (event.type === "turn_end") {
			if (!this._activeExchangeId || !this._activeModelTurnId || event.message.role !== "assistant") return;
			this._ensureActiveModelTurnStarted();
			const status = this._agentAbortRequested
				? "interrupted"
				: event.message.stopReason === "error"
					...
					: event.message.stopReason === "aborted"
						? "interrupted"
						: "completed";
			if (status === "interrupted") this._completeActiveAction("cancelled");
			this._projectExchange({
		...
		const delivery =
			this._messageDeliveries.get(event.message) ??
			(event.message.role === "user" ? this._inferQueuedDelivery(event.message) : undefined);
		if (delivery) this._messageDeliveries.set(event.message, delivery);
		if (event.message.role !== "user" && !delivery) {
			this._ensureActiveModelTurnStarted();
			if (event.message.role === "assistant") this._syncStreamingCommentary(event.message);
			return;
		}

		if (!this._activeExchangeId) {
			this._startExchange(event.message, "prompt");
		}
		if (delivery === "follow_up") {
			if (this._isActiveModelTurnProjected()) {
				this._addExchangeInput(event.message, "follow_up");
			} else {
				this._startExchange(event.message, "follow_up");
			}
		} else if (delivery === "steer") {
			this._addExchangeInput(event.message, "steer");
		}
		this._ensureActiveModelTurnStarted();
	}

	private _isActiveModelTurnProjected(): boolean {
		if (!this._activeExchangeId || !this._activeModelTurnId) return false;
		const exchange = this._exchangeProjection.exchanges.find((candidate) => candidate.id === this._activeExchangeId);
		return exchange?.modelTurns.some((turn) => turn.id === this._activeModelTurnId) ?? false;
	}

	private _ensureActiveModelTurnStarted(): void {
		if (!this._activeExchangeId || !this._activeModelTurnId) return;
		if (this._isActiveModelTurnProjected()) return;
		this._projectExchange({
	...
	private _addExchangeInput(message: AgentMessage, delivery: "steer" | "follow_up"): void {
		if (!this._activeExchangeId) return;
		this._projectExchange({
			type: "exchange_input_added",
			exchangeId: this._activeExchangeId,
			input: { id: randomUUID(), delivery, content: this._getExchangeInputText(message) },
			at: message.timestamp ?? Date.now(),
		});
	}

	private _startExchange(message: AgentMessage, delivery: "prompt" | "follow_up"): void {
		const previousExchangeId = this._exchangeProjection.activeExchangeId;
		if (previousExchangeId) {
			...
			this._queueSubagentExchangeClose(previousExchangeId);
		}
	...
	private _setAction(actionId: string, title: string): void {
		if (!this._activeExchangeId || !this._activeModelTurnId) return;
		this._completeActiveAction("completed");
		this._projectExchange({
			type: "action_started",
			exchangeId: this._activeExchangeId,
			actionId,
	...
	private _getActiveAction(): ActionItem | undefined {
		return this._getActiveExchange()?.items.find(
			(item): item is ActionItem => item.type === "action" && item.status === "in_progress",
		);
	}

	private _getActionForToolCall(toolCallId: string): ActionItem | undefined {
		return this._getActiveExchange()?.items.find(
			(item): item is ActionItem =>
				item.type === "action" && item.toolCalls.some((toolCall) => toolCall.id === toolCallId),
		);
	}

	private _completeActiveAction(status: "completed" | "failed" | "cancelled"): void {
		const action = this._getActiveAction();
		if (!action || !this._activeExchangeId) return;
		this._projectExchange({
	...
	private _actionStatusForExchange(): "completed" | "failed" | "cancelled" {
		const outcome = this._exchangeOutcome();
		return outcome === "failed" ? "failed" : outcome === "interrupted" ? "cancelled" : "completed";
	}

	private _exchangeOutcome(): "completed" | "failed" | "interrupted" {
		const status = this._getActiveExchange()?.modelTurns.at(-1)?.status;
		return status === "failed" || status === "interrupted" ? status : "completed";
	}

	private _getExchangeInputText(message: AgentMessage): string {
		if (message.role === "user") return this._getUserMessageText(message);
	...
	private _syncStreamingCommentary(message: AssistantMessage): void {
		if (!this._activeExchangeId || !this._activeModelTurnId) return;
		const content = message.content
			.filter((part): part is TextContent => part.type === "text" && getTextContentPhase(part) === "commentary")
			.map((part) => part.text)
			.join("\n");
		if (!content) return;
		const current = this._getStreamingCommentaryNarrative();
		if (!current) {
		...
		if (content === current.content || !content.startsWith(current.content)) return;
		this._projectExchange({
			type: "narrative_delta",
			exchangeId: this._activeExchangeId,
			narrativeId: current.id,
			delta: content.slice(current.content.length),
			at: Date.now(),
		});
	}

	private _completeAssistantNarratives(message: AssistantMessage, response: AgentProtocolResponse): void {
		if (!this._activeExchangeId || !this._activeModelTurnId) return;
		const isIncomplete =
			message.stopReason === "error" || message.stopReason === "aborted" || message.stopReason === "length";
		this._ensureActiveModelTurnStarted();
		this._syncStreamingCommentary(message);
		const streamingCommentary = this._getStreamingCommentaryNarrative();
		const streamedCommentary = streamingCommentary?.content ?? "";
		if (streamingCommentary) {
			this._projectExchange({
				type: "narrative_completed",
				exchangeId: this._activeExchangeId,
				narrativeId: streamingCommentary.id,
				status: isIncomplete ? "interrupted" : "completed",
				at: Date.now(),
			});
		}
		const commentary = response.commentary.startsWith(`${streamedCommentary}\n`)
			? response.commentary.slice(streamedCommentary.length + 1)
			: response.commentary === streamedCommentary
				? ""
				: response.commentary;
		for (const [phase, content] of [
			...
			if (!content) continue;
			if (phase === "final_answer") this._completeActiveAction("completed");
			const narrativeId = randomUUID();
			const at = Date.now();
			this._projectExchange({
				type: "narrative_started",
	...
	private _getStreamingCommentaryNarrative(): NarrativeItem | undefined {
		return this._getActiveExchange()?.items.find(
			(item): item is NarrativeItem =>
				item.type === "narrative" && item.phase === "commentary" && item.status === "streaming",
		);
	}

	private _projectExchange(event: ExchangeProjectorEvent): void {
		this._exchangeProjection = projectExchangeEvent(this._exchangeProjection, event);
		for (const listener of this._exchangeProjectionListeners) listener(this._exchangeProjection);
	}

	private _inferQueuedDelivery(message: Message): NonNullable<SessionMessageMetadata["delivery"]> | undefined {
		const text = this._getUserMessageText(message);
		if (!text) return undefined;
		if (this._steeringMessages.includes(text)) return "steer";
		if (this._followUpMessages.includes(text)) return "follow_up";
		return undefined;
	}

	private _getAssistantResponse(event: Extract<AgentEvent, { type: "message_end" }>): AgentProtocolResponse {
		if (isAgentProtocolResponse(event.responseMetadata)) return event.responseMetadata;
		if (event.responseMetadata !== undefined) {
			throw new Error("Agent protocol invariant violated: invalid assistant response metadata");
		}
		if (
			event.message.role !== "assistant" ||
			(event.message.stopReason !== "error" && event.message.stopReason !== "aborted")
		) {
			throw new Error("Agent protocol invariant violated: assistant response metadata is missing");
		}
		return classifyAgentProtocolResponse(event.message, {
			hasActiveAction: this._getActiveAction() !== undefined,
			hasPendingSteering: this._steeringMessages.length > 0,
		});
	}

	getCurrentExchangeMetadata(
		message?: AgentMessage,
		responseDisposition?: AgentProtocolResponse["disposition"],
	): SessionMessageMetadata | undefined {
		if (!this._activeExchangeId) return undefined;
		const delivery =
			message?.role === "user" || message?.role === "custom" ? this._messageDeliveries.get(message) : undefined;
		return {
			exchangeId: this._activeExchangeId,
			modelTurnId: this._activeModelTurnId,
			delivery,
	...
	/** Subscribe to immutable Exchange state without changing the legacy AgentSessionEvent stream. */
	subscribeExchangeProjection(listener: AgentSessionExchangeProjectionListener): () => void {
		this._exchangeProjectionListeners.push(listener);
		listener(this._exchangeProjection);
		return () => {
			const index = this._exchangeProjectionListeners.indexOf(listener);
			if (index !== -1) this._exchangeProjectionListeners.splice(index, 1);
		};
	}
	...
	dispose(): void {
		...
		this._exchangeProjectionListeners = [];
	...
	/** Close every child context before this session is replaced or discarded. */
	async shutdownSubagents(): Promise<void> {
		if (!this._subagentManager) return;
		if (!this._subagentShutdownPromise) {
			this._subagentShutdownPromise = this._subagentScopeCloseTail.then(
				async () => await this._subagentManager?.closeAll(),
			);
		}
		await this._subagentShutdownPromise;
	}

	private _queueSubagentExchangeClose(exchangeId: string): void {
		if (!this._subagentManager) return;
		this._subagentScopeCloseTail = this._subagentScopeCloseTail.then(
			async () => await this._subagentManager?.closeExchangeScoped(exchangeId),
		);
	}

	get subagentProjection(): SubagentProjection {
		return this._subagentManager?.projection ?? { executions: [] };
	}

	get subagentManager(): CodingSubagentManager | undefined {
		return this._subagentManager;
	}

	subscribeSubagentProjection(listener: SubagentProjectionListener): () => void {
		if (this._subagentManager) return this._subagentManager.subscribe(listener);
		listener({ executions: [] });
		return () => {};
	}

	resolveSubagentOrigin(toolCallId: string): SubagentOrigin | undefined {
		const exchange = this._getActiveExchange();
		const action = this._getActionForToolCall(toolCallId);
		if (!exchange || !action) return undefined;
		return { exchangeId: exchange.id, actionId: action.id, toolCallId };
	}
	...
	/** Product-level Exchange state projected from the live Pi lifecycle. */
	get exchangeProjection(): ExchangeProjection {
		return this._exchangeProjection;
	}
	...
	getActiveToolNames(): string[] {
		return this.agent.state.tools.map((t) => t.name).filter((name) => name !== SEMANTIC_ACTION_TOOL_NAME);
	}
	...
	private _restoreToolsAfterPlanMode(): void {
		const toolNames = this._toolsBeforePlanMode ?? [
			"read",
			"bash",
			"edit",
			"write",
	...
	getAllTools(): ToolInfo[] {
		return Array.from(this._toolDefinitions.values())
			.filter(({ definition }) => definition.name !== SEMANTIC_ACTION_TOOL_NAME)
	...
	setActiveToolsByName(toolNames: string[]): void {
		const requestedToolNames =
			this._collaborationMode === "plan"
				...
				: [...toolNames];
		if (!requestedToolNames.includes(SEMANTIC_ACTION_TOOL_NAME)) requestedToolNames.push(SEMANTIC_ACTION_TOOL_NAME);
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		preflightResult?.(true);
		const promptMessage = messages.find((message) => message.role === "user");
		if (promptMessage) {
			this._startExchange(promptMessage, "prompt");
			this._messageDeliveries.set(promptMessage, "prompt");
		}
	...
	private async _queueSteer(text: string, images?: ImageContent[]): Promise<void> {
		...
		this._messageDeliveries.set(message, "steer");
	...
	private async _queueFollowUp(text: string, images?: ImageContent[]): Promise<void> {
		...
		this._messageDeliveries.set(message, "follow_up");
	...
	async sendCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "nextTurn") {
			...
			if (options?.deliverAs === "followUp") {
				this._messageDeliveries.set(appMessage, "follow_up");
				this.agent.followUp(appMessage);
			} else {
				this._messageDeliveries.set(appMessage, "steer");
				this.agent.steer(appMessage);
			}
		} else if (options?.triggerTurn) {
			this._startExchange(appMessage, "prompt");
			this._messageDeliveries.set(appMessage, "prompt");
	...
	async abort(): Promise<void> {
		this.abortRetry();
		this._agentAbortRequested = true;
		this.agent.abort();
	...
	private _refreshToolRegistry(options?: { activeToolNames?: string[]; includeAllExtensionTools?: boolean }): void {
		...
		for (const tool of allCustomTools) {
			if (
				(tool.definition.name === "ask_user_question" || tool.definition.name === SEMANTIC_ACTION_TOOL_NAME) &&
				definitionRegistry.has(tool.definition.name)
			)
				continue;
		...
		for (const tool of wrappedExtensionTools as AgentTool[]) {
			if (
				(tool.name === "ask_user_question" || tool.name === SEMANTIC_ACTION_TOOL_NAME) &&
	...
	private _buildRuntime(options: {
		...
		if (this._subagentManager) {
			Object.assign(
				baseToolDefinitions,
				createSubagentToolDefinitions({
					manager: this._subagentManager,
					resolveOrigin: (toolCallId) => this.resolveSubagentOrigin(toolCallId),
				}),
			);
		}
		Object.assign(baseToolDefinitions, {
			[SEMANTIC_ACTION_TOOL_NAME]: createSemanticActionToolDefinition((toolCallId, title) =>
				this._setAction(toolCallId, title),
			),
		});
		...
		const defaultActiveToolNames = this._baseToolsOverride
			...
			: [
					...
					"ask_user_question",
					...FORGE_TOOL_NAMES,
					...(this._subagentManager ? SUBAGENT_TOOL_NAMES : []),
				];
	...
	recordBashResult(command: string, result: BashResult, options?: { excludeFromContext?: boolean }): void {
		...
		if (this.isStreaming) {
			...
			// Save to session
			this.sessionManager.appendMessage(bashMessage, this.getCurrentExchangeMetadata(bashMessage));
		}
	}
