...
- **Single-file GitHub Releases** - Download one self-contained executable per platform; Bone materializes its embedded runtime assets in a versioned user cache on first launch. See [Release Downloads](../../docs/releases.md).

### Changed

- Changed GitHub Release artifacts from multi-file archives to one self-contained executable per platform; embedded themes, templates, WASM, clipboard bindings, and semantic native libraries are materialized into a versioned user cache on first launch.

## [0.2.0] - 2026-07-26

### New Features

- Added reusable lockstep packages for [session persistence](../session/README.md), [RPC contracts](../protocol/README.md), [image processing](../images/README.md), [Forge integrations](../forge/README.md), and [semantic memory](../memory/README.md) while keeping them built into the standard Bone runtime.

### Breaking Changes

- Removed implicit global provider streams from Agent and compaction APIs; direct integrations must now provide the session `streamFn`.

### Changed

- Split session persistence, image/clipboard processing, Forge services, semantic memory/native embedding, and RPC contracts into required lockstep workspace packages; all remain built-in runtime dependencies rather than optional modules.
- Removed production dependencies on the global AI compatibility registry in favor of `ModelRuntime` and explicit built-in API implementations.

## [0.1.11] - 2026-07-26

### Added

- Added first-class in-process sub-agents with isolated persistent sessions, lifecycle and mailbox tools, exchange/conversation scoping, bounded handoffs and yields, read-only child defaults, and parent Action status/message projection in OpenTUI.

### Removed

- Removed the remaining external extension entry points, public extension SDK exports, examples, legacy package-manager implementation, and extension-defined CLI flag compatibility.

## [0.1.10] - 2026-07-25

### Breaking Changes

- Removed `semantic_action_start` and `semantic_action_end` from `AgentSessionEvent`; consume `set_action` tool events or `exchangeProjection` instead.
- Renamed internal Exchange projector events from `semantic_action_started`/`semantic_action_completed` to `action_started`/`action_completed`.
- Replaced session migrations with strict session format version 4 loading; older versions, malformed JSONL, and invalid Exchange metadata are rejected.

### Changed

- Made OpenTUI consume Exchange projections as the source of truth for active Action identity, ToolCall ownership, completion status, and foreground Action recovery.
- Made assistant response classification explicit `message_end` metadata instead of retaining it by AssistantMessage object identity.
- Made memory derive completed work only from persisted Exchange metadata and removed the legacy ungrouped-message completion fallback.

### Fixed

- Prevented rejected or steering-overridden assistant responses from completing exchange memory.
...
- Added governed GitLab CI/CD and GitHub Actions variable listing, single-value reads, and create, update, and delete operations with value-safe summaries and mutation receipts.

### Changed

- Added concise tool-defined transcript summaries and structured Forge call receipts for collapsed tool activity.
- Added semantic Actions that can span ModelTurns and group multiple ToolCalls, with multiple Actions allowed under one normal-prose stage explanation and the rolling highlight reserved for the active Action or the temporary `Working` state when no Action is active.
- Made completed Actions static and collapsed, with Action clicks revealing ToolCall summaries and ToolCall clicks revealing full parameters and results; failed ToolCalls now stay collapsed and render in red instead of auto-expanding.
- Enforced Action declarations before ordinary tool execution: invalid response batches now execute no tools, return hidden correction feedback to the model, and fail explicitly after the bounded correction limit instead of creating inferred live Actions.

### Fixed

- Restored `Esc` as the active Agent interrupt key while keeping `Ctrl+C` scoped to clearing the composer.
...
- Replaced modal structured-question sequences with one persistent, non-modal answer panel above the composer.
- Moved Plan approval and revision feedback into a persistent, non-modal review panel above the composer.
- Moved model, provider authentication, logout, and workspace-trust choices into lightweight pickers above the composer.
...
- Replaced animated aggregate Agent activity with persistent phase updates followed by directly visible, independently expandable tool actions.
...
- Restored hover feedback and pointer cursors for clickable conversation rows, tool summaries, and transcript update notices.
...
- Fixed structured-question focus recovery after submitting an answer or cancelling the questionnaire.
...
- Improved structured questions so selected options can include supplemental custom text, question tabs no longer repeat their headers in the body, and option details keep a stable height while navigating.
...
- pi-ai's old global API (`stream`/`complete`/`completeSimple`, `getModel`/`getModels`/`getProviders`, `registerApiProvider`, `getEnvApiKey`, ...) moved off the `@frelion/bone-ai` root entrypoint to `@frelion/bone-ai/compat`. Extensions are not affected at runtime: the extension loader resolves the pi-ai root to the compat entrypoint (a strict superset), so existing extensions keep working unchanged. Extension sources that typecheck against pi-ai's published types should switch those imports to `@frelion/bone-ai/compat` (or migrate to the new `createModels()`/provider-factory API). The compat entrypoint and the loader alias will be removed in a future release with a migration guide.
...
- Removed inherited selective-provider `@frelion/bone-ai/base` and `@frelion/bone-agent-core/base` entrypoints; use the root packages with explicit `Models` provider factories instead.
...
- Added inherited `@frelion/bone-ai/base` and `@frelion/bone-agent-core/base` entry points for selective provider registration in bundled applications ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
...
- Added Claude Fable 5 model support inherited from `@frelion/bone-ai` for the Anthropic and Amazon Bedrock providers, with adaptive thinking and `xhigh` effort support.
...
- Added MiniMax-M3 model support inherited from `@frelion/bone-ai` for the `minimax` and `minimax-cn` direct providers ([#5313](https://github.com/earendil-works/pi/issues/5313)).
...
- Added custom Amazon Bedrock request header support inherited from `@frelion/bone-ai` ([#5178](https://github.com/earendil-works/pi-mono/pull/5178) by [@stephanmck](https://github.com/stephanmck)).
...
- Fixed Amazon Bedrock provider loading under strict package managers by inheriting the declared `@smithy/node-http-handler` dependency from `@frelion/bone-ai` ([#4842](https://github.com/earendil-works/pi/issues/4842)).
...
- Fixed loader indicator startup inherited from `@frelion/bone-tui` so initialization cannot run before frames are available.
- Fixed OpenAI-compatible default output token requests inherited from `@frelion/bone-ai` to avoid reserving impossible context windows on servers such as vLLM ([#4675](https://github.com/earendil-works/pi/issues/4675)).
- Fixed OpenAI prompt cache keys inherited from `@frelion/bone-ai` to stay within the 64-character provider limit ([#4720](https://github.com/earendil-works/pi/issues/4720)).
...
- Fixed Xiaomi MiMo generated model metadata to replay assistant tool-call messages with `reasoning_content` for thinking-mode multi-turn requests, inherited from `@frelion/bone-ai` ([#4678](https://github.com/earendil-works/pi/issues/4678)).
...
- Fixed Anthropic-compatible API-key requests to ignore unrelated `ANTHROPIC_AUTH_TOKEN` environment values, avoiding invalid bearer credentials for providers such as Xiaomi MiMo inherited from `@frelion/bone-ai` ([#4342](https://github.com/earendil-works/pi/issues/4342)).
- Fixed Amazon Bedrock message conversion to skip unknown content blocks instead of failing the stream, inherited from `@frelion/bone-ai` ([#4223](https://github.com/earendil-works/pi/issues/4223)).
- Fixed Azure OpenAI Responses and OpenAI Responses error formatting to prefix HTTP status codes onto `errorMessage`, so transient 5xx and 429 errors are correctly matched by the agent-level auto-retry classifier inherited from `@frelion/bone-ai` ([#4232](https://github.com/earendil-works/pi/issues/4232)).
- Fixed OpenCode Go Kimi reasoning replay by normalizing streamed `reasoning` fields back to `reasoning_content` for OpenCode Go only, inherited from `@frelion/bone-ai` ([#4251](https://github.com/earendil-works/pi/issues/4251)).
- Fixed Xiaomi MiMo model metadata to use the OpenAI-compatible endpoints and `openai-completions` API, restoring multi-turn thinking/tool-call sessions inherited from `@frelion/bone-ai` ([#4505](https://github.com/earendil-works/pi/issues/4505)).
...
- Removed non-working OpenAI Codex fast model variants inherited from `@frelion/bone-ai`.
...
- Fixed OpenAI Codex generated model metadata to use the current upstream model list inherited from `@frelion/bone-ai` ([#4603](https://github.com/earendil-works/pi-mono/pull/4603) by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed GitHub Copilot GPT model thinking metadata inherited from `@frelion/bone-ai` to map unsupported minimal thinking to low ([#4622](https://github.com/earendil-works/pi-mono/pull/4622) by [@mattiacerutti](https://github.com/mattiacerutti)).
...
- Fixed default output token requests for models whose advertised output limit is effectively their full context window, avoiding impossible provider requests inherited from `@frelion/bone-ai` ([#4614](https://github.com/earendil-works/pi/issues/4614)).

## [0.74.1] - 2026-05-16

### New Features

- **Image generation support** - Added image generation APIs, generated image model metadata, and built-in OpenRouter image generation support inherited from `@frelion/bone-ai`.
...
- **Improved terminal and markdown rendering** - Added markdown list indentation, task-list checkbox rendering, large markdown robustness, and inline image placement fixes inherited from `@frelion/bone-tui`.

### Added

- Added image generation support from `@frelion/bone-ai`, including image generation APIs, image model metadata, and built-in OpenRouter image generation support ([#3887](https://github.com/earendil-works/pi-mono/pull/3887) by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- Fixed OpenRouter cached-token usage accounting, Fireworks caching compatibility, and OpenAI Codex WebSocket proxy handling inherited from `@frelion/bone-ai`.
- Fixed markdown list wrapping, task-list checkboxes, large markdown rendering, WezTerm Kitty keyboard escape handling, and short-viewport inline image placement inherited from `@frelion/bone-tui`.
...
- **Forking/Rebranding Support**: All branding (app name, config directory, environment variable names) is now configurable via `boneConfig` in `package.json`. Forks can change `boneConfig.name` and `boneConfig.configDir` to rebrand the CLI without code changes. Affects CLI banner, help text, config paths, and error messages. ([#95](https://github.com/badlogic/pi-mono/pull/95))
