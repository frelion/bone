...
describe("parseArgs", () => {
	...
	describe("removed extension flags", () => {
		test("rejects extension flags as unknown options", () => {
			const result = parseArgs(["--extension", "./my-extension.ts", "--no-extensions"]);
			expect(result.diagnostics).toEqual([
				{ type: "error", message: "Unknown option: --extension" },
				{ type: "error", message: "Unknown option: --no-extensions" },
			]);
			expect(result.messages).toEqual(["./my-extension.ts"]);
		});
	});
	...
	describe("messages and file args", () => {
		...
		test("rejects unknown long flags without consuming following messages", () => {
		...
		test("rejects unknown boolean long flags", () => {
			const result = parseArgs(["--unknown-flag"]);
			expect(result.diagnostics).toContainEqual({ type: "error", message: "Unknown option: --unknown-flag" });
		});

		test("rejects unknown long flags with equals syntax", () => {
			const result = parseArgs(["--unknown-flag=value"]);
			expect(result.diagnostics).toContainEqual({
				type: "error",
				message: "Unknown option: --unknown-flag=value",
			});
		});
	});
