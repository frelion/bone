...
import type { AssistantMessage, Usage } from "@frelion/bone-ai/compat";
...
function loadLargeSessionEntries(): SessionEntry[] {
	...
	return entries.flatMap((entry, index): SessionEntry[] => {
		if (entry.type === "session") return [];
		const id = `fixture-${index}`;
		const normalized = { ...entry, id, parentId } as SessionEntry;
		parentId = id;
		return [normalized];
	});
}
