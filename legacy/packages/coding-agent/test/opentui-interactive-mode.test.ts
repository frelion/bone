...
import type { SubagentProjection } from "../src/core/subagents/index.ts";
...
interface FakeSession {
	...
	readonly exchangeProjection: ExchangeProjection;
	readonly subagentProjection: SubagentProjection;
	subscribe(listener: AgentSessionEventListener): () => void;
	subscribeExchangeProjection(listener: (projection: ExchangeProjection) => void): () => void;
	subscribeSubagentProjection(listener: (projection: SubagentProjection) => void): () => void;
	abort(): Promise<void>;
...
function createRuntime(
	...
	const projectionListeners = new Set<(projection: ExchangeProjection) => void>();
	const subagentProjectionListeners = new Set<(projection: SubagentProjection) => void>();
	let projection = createExchangeProjection(metadata.id ?? "session-id");
	let subagentProjection: SubagentProjection = { executions: [] };
	...
	const session: FakeSession = {
		...
		get exchangeProjection() {
			return projection;
		},
		get subagentProjection() {
		...
		subscribeExchangeProjection: (listener) => {
			projectionListeners.add(listener);
			listener(projection);
			return () => projectionListeners.delete(listener);
		},
		subscribeSubagentProjection: (listener) => {
			subagentProjectionListeners.add(listener);
			listener(subagentProjection);
			return () => subagentProjectionListeners.delete(listener);
		},
		abort,
	...
	const publishProjection = (next: ExchangeProjection): void => {
		projection = next;
		for (const listener of projectionListeners) listener(projection);
	};
	const projectEvent = (event: AgentSessionEvent): void => {
		if (event.type === "tool_execution_start" && event.toolName === "set_action") {
			const title =
			...
			if (!title) return;
			const exchange =
				projection.exchanges.find((candidate) => candidate.id === projection.activeExchangeId) ??
				({
					id: "test-exchange",
					...
					items: [],
					startedAt: 0,
				} satisfies ExchangeProjection["exchanges"][number]);
			const items = [
				...exchange.items.map((item) =>
					item.type === "action" && item.status === "in_progress"
			...
			const nextExchange = { ...exchange, items };
			publishProjection({
				sessionId: projection.sessionId,
				activeExchangeId: nextExchange.id,
				exchanges: projection.exchanges.some((candidate) => candidate.id === nextExchange.id)
					? projection.exchanges.map((candidate) => (candidate.id === nextExchange.id ? nextExchange : candidate))
					: [...projection.exchanges, nextExchange],
			});
			return;
		}
		const activeExchange = projection.exchanges.find((candidate) => candidate.id === projection.activeExchangeId);
		if (!activeExchange) return;
		if (event.type === "tool_execution_start") {
			const items = activeExchange.items.map((item) => {
				if (item.type !== "action" || item.status !== "in_progress") return item;
				const toolCall: ToolCallExecution = {
					id: event.toolCallId,
					modelTurnId: "test-turn",
					toolName: event.toolName,
					status: "in_progress",
					arguments: event.args,
					sequence: item.toolCalls.length,
					startedAt: Date.now(),
				};
				return { ...item, toolCalls: [...item.toolCalls, toolCall] };
			});
			publishProjection({
				...projection,
				exchanges: projection.exchanges.map((candidate) =>
					candidate.id === activeExchange.id ? { ...activeExchange, items } : candidate,
				),
			});
			return;
		}
		if (event.type === "tool_execution_end" && event.toolName !== "set_action") {
			const items = activeExchange.items.map((item) =>
				item.type === "action"
					? {
							...item,
	...
	return {
		...
		projectionListenerCount: () => projectionListeners.size,
		subagentProjectionListenerCount: () => subagentProjectionListeners.size,
		publishSubagentProjection: (next: SubagentProjection) => {
			subagentProjection = next;
			for (const listener of subagentProjectionListeners) listener(next);
		},
	};
}
...
describe("OpenTUIInteractiveMode", () => {
	...
	test("updates streaming assistant and tool output without rebuilding the shell", async () => {
		...
		session.emit({
			...
			toolCallId: "action-1",
			toolName: "set_action",
			args: { title: "read · README.md · 24 lines" },
		});
		session.emit({
		...
		const expandedSummaryRows = frame
			.split("\n")
			.map((line, index) => (line.includes(summary) ? index : -1))
			.filter((index) => index >= 0);
		expect(expandedSummaryRows).toHaveLength(2);
		await renderer.mouse.click(7, expandedSummaryRows[1]);
		await new Promise<void>((resolve) => setImmediate(resolve));
		await settle(renderer);
		frame = renderer.captureFrame();
	...
	test("subscribes to child-agent projection updates and renders them in the owning Action", async () => {
		...
		active.emit({
			type: "tool_execution_start",
			toolCallId: "delegated-action",
			toolName: "set_action",
			args: { title: "Reviewing concurrency" },
		});
		active.publishSubagentProjection({
			executions: [
				{
					agentRef: "child-1",
		...
		expect(frame).toContain("Reviewing concurrency");
		expect(frame).toContain("◐ running");
		expect(frame).toContain("Runtime reviewer");
		mode.stop();
	});

	test("resumes a persisted active Action before handling foreground live tools", async () => {
		const actionMessage = {
			...
			content: [
				{
					...
					id: "persisted-action",
					name: "set_action",
					arguments: { title: "Inspecting persisted state" },
				},
			],
		};
		const session = createRuntime([
			entry(actionMessage, "persisted-action-message"),
			entry(
				{
					role: "toolResult",
					toolName: "set_action",
					toolCallId: "persisted-action",
					content: [{ type: "text", text: "Action active." }],
					isError: false,
					timestamp: 3,
				},
		...
		const frame = renderer.captureFrame();
		expect(frame).toContain("Inspecting persisted state");
		expect(frame).not.toContain("Agent protocol invariant violated");
		mode.stop();
	});
	...
	test("replays runtime events emitted while transcript history is loading", async () => {
		...
		const renderer = await createNativeTestRenderer({ width: 90, height: 24 });
		const transcriptFactory = new OpenTUITranscriptFactory(renderer);
		const originalHandleEvent = transcriptFactory.handleEvent.bind(transcriptFactory);
		...
		const handleEvent = vi.spyOn(transcriptFactory, "handleEvent").mockImplementation(async (event) => {
		...
		active.emit({
			type: "tool_execution_start",
			toolCallId: "buffered-action",
			toolName: "set_action",
			args: { title: "Buffered replay action" },
		});
		active.emit({
			type: "tool_execution_end",
			toolCallId: "buffered-action",
		...
		active.emit({
			type: "tool_execution_start",
			toolCallId: "buffered-read",
			toolName: "read",
			args: { path: "buffered.ts" },
		});
		active.emit({
			type: "tool_execution_end",
			toolCallId: "buffered-read",
			toolName: "read",
			result: { content: [{ type: "text", text: "buffered result" }], details: {} },
			isError: false,
		});
		...
		expect(handleEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "message_update" }));
		expect(handleEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "agent_settled" }));
		expect(renderer.captureFrame()).toContain("Buffered replay action");
		mode.stop();
	});
	...
	test("rebinds foreground subscriptions and handles fixed clear, interrupt, and shutdown keys", async () => {
		...
		expect(first.projectionListenerCount()).toBe(1);
		expect(first.subagentProjectionListenerCount()).toBe(1);
		...
		expect(first.projectionListenerCount()).toBe(0);
		expect(first.subagentProjectionListenerCount()).toBe(0);
		expect(second.listenerCount()).toBe(1);
		expect(second.projectionListenerCount()).toBe(1);
		expect(second.subagentProjectionListenerCount()).toBe(1);
		await settle(renderer);
		...
		await settle(renderer);
		expect(renderer.captureFrame()).not.toContain("draft");
	...
	test("collects structured answers in one non-modal panel and restores composer focus", async () => {
		...
		expect(questionFrame).toContain("> 1 Runtime");
		expect(questionFrame).toContain("2 Scope");
		expect(questionFrame).toContain("3 Notes");
		expect((mode as unknown as { overlayManager: { active: unknown } }).overlayManager.active).toBeFalsy();
		renderer.input.pressEnter();
