...
describe("OpenTUIQuestionnaire", () => {
	test("renders distinct tabs with option details in a right-hand pane", async () => {
		...
		expect(frame).toContain("> 1 Build");
		expect(frame).toContain("2 Targets");
		expect(frame).toContain("Options");
		expect(frame).toContain("Details");
		expect(frame).toContain("Local");
		expect(frame).toContain("Only local development");
		expect(frame).not.toContain("Local Only local development");
		expect(frame.split("\n").find((line) => line.includes("Options"))).toContain("│ Details");
		const tabLine = frame.split("\n").find((line) => line.includes("> 1 Build")) ?? "";
		expect(tabLine.indexOf("2 Targets")).toBeLessThan(24);
		expect(tabLine.indexOf("Review")).toBeLessThan(40);

		questionnaire.handleKey(key("enter"));
		questionnaire.handleKey(key("right"));
		await setup.flush();
		const nextQuestionFrame = setup.captureCharFrame();
		expect(nextQuestionFrame).toContain("✓ 1 Build");
		expect(nextQuestionFrame).toContain("> 2 Targets");
	});

	test("keeps left and right tab navigation at the first and last tabs", async () => {
		const setup = await createQuestionnaireRenderer();
		const questionnaire = new OpenTUIQuestionnaire(setup.renderer, request(), vi.fn());
		setup.renderer.root.add(questionnaire.root);

		questionnaire.handleKey(key("left"));
		await setup.flush();
		expect(setup.captureCharFrame()).toContain("> 1 Build");
