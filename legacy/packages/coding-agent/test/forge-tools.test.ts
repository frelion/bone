...
describe("Forge built-in tools", () => {
	it("exposes closed operation-specific schemas that reject placeholder bags", () => {
		...
		expect(
			Value.Check(definitions.forge_variable.parameters, {
				action: "update",
				requestId: "rotate-deploy-env",
				variableKey: "DEPLOY_ENV",
				input: { value: "secret-value", protected: true, masked: true },
			}),
		).toBe(true);
		expect(
			Value.Check(definitions.forge_variable.parameters, {
	...
	it("provides concise structured summaries for Forge calls and results", () => {
		const definitions = createForgeToolDefinitions({ cwd: "/workspace", service: { execute: vi.fn() } });
		const issueSummary = definitions.forge_issue.renderV2?.summarize;
		const querySummary = definitions.forge_query.renderV2?.summarize;
		expect(issueSummary).toBeTypeOf("function");
		expect(querySummary).toBeTypeOf("function");

		expect(
			issueSummary?.({
				args: { action: "create", requestId: "issue-1", input: { title: "Fix it" } },
				phase: "running",
				isError: false,
			}),
		).toBe("Forge issue · creating · running");
		expect(
			issueSummary?.({
				args: { action: "create", requestId: "issue-1", input: { title: "Fix it" } },
				phase: "complete",
				...
				result: {
					content: [],
					details: { ok: true, resource: "issue", action: "create", number: 42, state: "open" },
				},
			}),
		).toBe("Forge issue · created #42 · open");
		expect(
			querySummary?.({
				args: { operation: "list", resource: "issue", state: "open" },
				phase: "complete",
				isError: false,
				result: {
					content: [],
					details: { resource: "issue", mode: "list", returned: 3, items: [] },
				},
			}),
		).toBe("Forge query · listed 3 issues");
		const variableSummary = definitions.forge_variable.renderV2?.summarize;
		expect(
			variableSummary?.({
				args: {
					action: "update",
					...
					input: { value: "must-not-appear" },
				},
				phase: "complete",
				isError: false,
				result: { content: [], details: { ok: true, resource: "variable", action: "update", key: "DEPLOY_TOKEN" } },
			}),
		).toBe("Forge variable · updated DEPLOY_TOKEN · complete");
	});
