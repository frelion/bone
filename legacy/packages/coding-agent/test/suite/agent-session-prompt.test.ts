...
describe("AgentSession prompt characterization", () => {
	...
	it("handles a tool call turn and waits for the follow-up LLM response", async () => {
		...
		harness.setResponses([
			stageResponse("I will run the echo tool and report its result.", "Running the echo tool"),
		...
		expect(harness.session.messages[4]?.role).toBe("toolResult");
	...
	it("executes multiple tool calls from one response and continues with a single follow-up response", async () => {
		...
		harness.setResponses([
			stageResponse("I will run both tools and compare their results.", "Running the requested tools"),
		...
		expect(
			harness.session.messages.filter(
				(message) => message.role === "toolResult" && message.toolName !== "set_action",
			),
		).toHaveLength(2);
		expect(harness.session.messages[harness.session.messages.length - 1]?.role).toBe("assistant");
		const actions = harness.session.exchangeProjection.exchanges[0]?.items.filter((item) => item.type === "action");
		expect(actions).toHaveLength(1);
		expect(actions?.[0]?.toolCalls).toEqual([
			expect.objectContaining({ toolName: "slow", status: "completed" }),
			expect.objectContaining({ toolName: "fast", status: "completed" }),
		]);
	});
	...
	it("reports streamingBehavior to input handlers while streaming", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will wait while the follow-up is queued.", "Waiting for queued input"),
	...
	it("throws when prompted during streaming without a streamingBehavior", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will wait while the prompt remains active.", "Waiting for prompt completion"),
			fauxAssistantMessage(fauxToolCall("wait", {}), { stopReason: "toolUse" }),
			fauxAssistantMessage("done"),
		]);
