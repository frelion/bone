...
describe("AgentSession structured questions", () => {
	...
	it("pauses on ask_user_question and continues after an answer", async () => {
		...
		harness.setResponses([
			stageResponse("I will ask for the choice needed to continue.", "Collecting the requested choice"),
	...
	it("persists an identifiable no-ui request in print mode", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will ask which mode should be used.", "Collecting the preferred mode"),
			fauxAssistantMessage(
