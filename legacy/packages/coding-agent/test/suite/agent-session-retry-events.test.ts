...
import { createHarness, getAssistantTexts, type Harness, stageResponse } from "./harness.ts";
...
describe("AgentSession retry and event characterization", () => {
	...
	it("waits for the full loop when retry recovery produces tool calls", async () => {
		...
		harness.setResponses([
			fauxAssistantMessage("", { stopReason: "error", errorMessage: "overloaded_error" }),
			stageResponse("I will run the recovered tool call before replying.", "Running the recovered tool call"),
	...
	it("retries a stream read failure after a completed tool without running the tool again", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will run the tool once before handling any retry.", "Running the echo tool"),
			fauxAssistantMessage([fauxToolCall("echo", { text: "hello" })], { stopReason: "toolUse" }),
	...
	it("emits the expected event order for a tool call turn", async () => {
		...
		expect(normalizeEventOrder(harness.events)).toEqual([
			...
			"message_update",
			"message_end:assistant",
			"tool_execution_start:set_action",
			"tool_execution_end:set_action",
	...
	it("emits streaming deltas for text, thinking, and tool calls in message_update events", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will inspect the streaming event blocks.", "Inspecting streaming events"),
