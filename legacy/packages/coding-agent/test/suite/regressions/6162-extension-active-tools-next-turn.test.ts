...
import { createHarness, stageResponse } from "../harness.ts";
...
describe("extension active tools next-turn refresh", () => {
	it("applies pi.setActiveTools before the next provider request in the same run", async () => {
		...
		try {
			...
			harness.setResponses([
				stageResponse("I will switch the active extension tools.", "Switching extension tools"),
				(context) => {
			...
			expect(providerToolNames).toEqual([
				["set_action", "switch_tools"],
				["after_switch", "set_action"],
			]);
		} finally {
			harness.cleanup();
		}
	...
	it("records additive active tool changes on the current tool result", async () => {
		...
		try {
			...
			harness.setResponses([
				stageResponse("I will load the additional extension tool.", "Loading extension tools"),
	...
	it("preserves before_agent_start system prompt overrides when tools change mid-run", async () => {
		...
		try {
			...
			const providerToolNames: string[][] = [];
			harness.setResponses([
				stageResponse("I will switch tools while retaining the system prompt.", "Switching extension tools"),
				(context) => {
