...
import { createHarness, type Harness, stageResponse } from "../harness.ts";
...
describe("regressions #1717/#2113: agent session event settlement", () => {
	...
	it("keeps persisted assistant/toolResult message order when extension message_end handlers yield", async () => {
		...
		harness.setResponses([
			stageResponse("I will run both echo calls and preserve their order.", "Running echo calls"),
	...
	it("runs tool_call handlers after the assistant tool-use message is settled in the session", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will run the echo call after declaring the Action.", "Running the echo call"),
		...
		expect(branchRolesAtToolCall).toEqual([["user", "assistant", "toolResult", "assistant"]]);
	});
});
