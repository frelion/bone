...
import { createExtensionUIV2Context, type ExtensionUIV2Context } from "../../src/core/extensions/ui-v2.ts";
import type { BuildSystemPromptOptions } from "../../src/index.ts";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		harness.setResponses([
			stageResponse(
				"I will invoke the extension tool and inspect the blocked result.",
				"Checking tool interception",
			),
	...
	it("allows extension tool_result handlers to modify tool results", async () => {
		...
		harnesses.push(harness);
		harness.setResponses([
			stageResponse("I will invoke the extension tool and inspect its result.", "Checking result interception"),
			fauxAssistantMessage([fauxToolCall("echo", { text: "hello" })], { stopReason: "toolUse" }),
			(context) => {
				const toolResult = context.messages.find(
					(message) => message.role === "toolResult" && message.toolName === "echo",
