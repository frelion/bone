...
const WAIT_STAGE_TEXT = "I will wait for queued messages before continuing.";
...
describe("AgentSession queue characterization", () => {
	...
	it("delivers extension-origin steering messages before the next LLM call", async () => {
		...
		harness.setResponses([
			stageResponse(WAIT_STAGE_TEXT, "Waiting for queued messages"),
	...
	it("delivers multiple steering messages in order in one-at-a-time mode", async () => {
		...
		expect(getAssistantTexts(harness)).toEqual([WAIT_STAGE_TEXT, "", "handled steer 1", "handled steer 2"]);
	});
	...
	it("delivers all steering messages in one batch in all mode", async () => {
		...
		expect(getAssistantTexts(harness)).toEqual([WAIT_STAGE_TEXT, "", "batched steer response"]);
	});
