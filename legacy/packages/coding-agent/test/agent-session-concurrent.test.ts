		...
		const agent = new Agent({
			...
			streamFn: async (_model, context) => {
				...
				queueMicrotask(() => {
					...
					if (toolResults.some((message) => message.toolName === "dummy")) {
					...
					const message: AssistantMessage = {
						role: "assistant",
						content: toolResults.some((result) => result.toolName === "set_action")
							? [
									{ type: "toolCall", id: "toolu_1", name: "dummy", arguments: { q: "x" } },
									{ type: "toolCall", id: "toolu_2", name: "dummy", arguments: { q: "y" } },
								]
							: [
									{ type: "text", text: "I will run both dummy calls." },
									{
										type: "toolCall",
										id: "action_1",
										name: "set_action",
										arguments: { title: "Running dummy calls" },
									},
								],
						api: "anthropic-messages",
						provider: "anthropic",
				...
				queueMicrotask(() => {
					...
					const message: AssistantMessage = {
						...
						content: toolResults.some((result) => result.toolName === "set_action")
							? [{ type: "toolCall", id: "toolu_1", name: "dummy", arguments: { q: "x" } }]
							: [
									{ type: "text", text: "I will call the dummy tool." },
									{
										type: "toolCall",
										id: "action_1",
										name: "set_action",
										arguments: { title: "Calling the dummy tool" },
									},
								],
						api: "anthropic-messages",
						provider: "anthropic",
