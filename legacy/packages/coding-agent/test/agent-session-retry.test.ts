...
import { Agent, type AgentEvent, type AgentTool } from "@frelion/bone-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@frelion/bone-ai/compat";
...
describe("AgentSession retry", () => {
	...
	it("prompt waits for full agent loop when retry produces tool calls", async () => {
			...
			streamFn: () => {
				...
				queueMicrotask(() => {
					if (callCount === 1) {
						...
						// Second call (retry): declare the Action before ordinary tools.
						const msg: AssistantMessage = {
							...
							content: [
								{ type: "text", text: "Looking that up now." },
								{
									...
									id: "action_1",
									name: "set_action",
									arguments: { title: "Looking up the answer" },
								},
							],
						};
						stream.push({ type: "start", partial: msg });
						...
						const msg: AssistantMessage = {
							...
							stopReason: "toolUse",
							content: [{ type: "toolCall", id: "call_1", name: "echo", arguments: { text: "hello" } }],
						};
						stream.push({ type: "start", partial: msg });
						stream.push({ type: "done", reason: "toolUse", message: msg });
					} else {
		...
		// Error, Action declaration, ordinary tool, and final response must all complete.
