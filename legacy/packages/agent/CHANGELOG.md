...
- Removed the implicit global provider stream from `Agent` and the low-level agent loop; callers that execute prompts must now provide `streamFn` explicitly.

## [0.1.11] - 2026-07-26

### Added

- Added a transport-neutral sub-agent runtime contract, bounded model-facing tools, an in-process implementation with persistent handles, follow-up questions, and non-blocking bounded yield mailboxes, plus an AgentHarness session adapter.

## [0.1.10] - 2026-07-25

### Changed

- Run assistant-response preflight before emitting `message_end` so persistence observes the validated response state.
- Forward assistant-response classification metadata from preflight to the corresponding `message_end` event.
...
- Removed the `@frelion/bone-agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@frelion/bone-agent-core/base` for bundlers that want to pair the agent core with selective `@frelion/bone-ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
