...
This document records the model/provider refactor design and its implementation history. The canonical current behavior is the explicit `Models`/provider architecture; legacy compat notes below are historical unless marked current.
...
- Old global APIs and the public `/compat` entrypoint are removed.
...
packages/ai/src/
  ...
  compat.ts                   # test-only legacy behavior harness; not exported
...
Provider and API implementations use explicit subpath exports. The compat subpath is no longer exported.
...
The public `@frelion/bone-ai/compat` entrypoint has been removed. Production code uses explicit `Models` collections, provider factories, and provider-owned authentication. `src/compat.ts` remains temporarily as a test-only harness for legacy behavior and is not exported by the package.
...
1. Main `@frelion/bone-ai` import is core-only.
...
8. `package.json` lists only active effectful modules in `sideEffects`; root and provider modules stay tree-shakeable.
...
- coding-agent uses `ModelRuntime` and explicit provider composition; production code no longer imports the old global AI API.
- External extension loading and its compat alias have been removed.
...
- [x] Removed the public compat subpath and its production side effect; retained `src/compat.ts` only for legacy test coverage.
- [x] Browser smoke and production examples use canonical Models/provider APIs.
...
- [x] Migrated production old-global imports to explicit Models/provider APIs and removed the extension compat alias.
...
- [x] `packages/coding-agent/CHANGELOG.md`: records the production migration to explicit Models/provider APIs and removal of external extension compatibility.
...
8. [ ] Rebuild coding-agent `ModelRegistry` over `MutableModels`.
   ...
   - Legacy `registerApiProvider` writes are absent from production; the old registry remains test-only.
9. [ ] Rewire consumers.
   ...
   - External extension loading and the root-to-compat alias have been removed.
...
### Phase 10 — compat deletion

- [x] Production Agent and AgentSession streaming use explicit runtime stream functions and Models/provider composition.
- [x] Moved all production and example imports off `/compat`.
- [x] Removed the public `/compat` package export and extension-loader alias. `src/compat.ts` remains test-only until legacy behavior tests are migrated.

### Deferred / follow-ups

- [ ] Web OAuth implementations (sitegeist-style) as an alternative `OAuthAuth`.
- [x] Images API redesign: `ImagesModels`/`ImagesProvider`/`createImagesProvider` mirror the chat-side design (sync reads, explicit refresh, never-reject generation); auth resolution shared with the chat side via the free-standing `resolveProviderAuth()` in `auth/resolve.ts` (which also owns `ModelsError`; both collections pass their store/context as arguments — no resolver object). `openrouterImagesProvider()` factory + `builtinImagesProviders()`/`builtinImagesModels()` in `providers/all`; impl moved to `api/openrouter-images.ts` with a lazy wrapper. The old global image API and compat package entrypoint are removed; `ImagesProvider` id alias in types.ts renamed to `ImagesProviderId` (mirror of `Provider` -> `ProviderId`).
