...
import {
	...
	type BeforeAssistantResponseResult,
	type StreamFn,
} from "./types.ts";
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			const responsePreflight =
				message.stopReason === "error" || message.stopReason === "aborted"
					? undefined
					: await preflightAssistantResponse(currentContext, message, toolCalls, config, signal);
			await emit({
				type: "message_end",
				message,
				...(responsePreflight?.responseMetadata !== undefined
					? { responseMetadata: responsePreflight.responseMetadata }
					: {}),
			});
			...
			if (responsePreflight?.block) {
				if (toolCalls.length > 0) {
					const rejectedBatch = await rejectToolCallsBeforeExecution(toolCalls, responsePreflight, emit);
					toolResults.push(...rejectedBatch.messages);
					for (const result of toolResults) {
						currentContext.messages.push(result);
						newMessages.push(result);
					}
					hasMoreToolCalls = !rejectedBatch.terminate;
				} else if (!responsePreflight.terminate && responsePreflight.correctionMessage) {
					await emit({ type: "message_start", message: responsePreflight.correctionMessage });
					await emit({ type: "message_end", message: responsePreflight.correctionMessage });
					currentContext.messages.push(responsePreflight.correctionMessage);
					newMessages.push(responsePreflight.correctionMessage);
			...
			if (responsePreflight?.block && (responsePreflight.terminate || !hasMoreToolCalls)) {
				const failureMessage = createPreflightFailureMessage(
					message,
					responsePreflight.reason || "Assistant response validation failed",
				);
...
async function preflightAssistantResponse(
	currentContext: AgentContext,
	assistantMessage: AssistantMessage,
	toolCalls: AgentToolCall[],
	config: AgentLoopConfig,
	signal: AbortSignal | undefined,
): Promise<BeforeAssistantResponseResult | undefined> {
	if (!config.beforeAssistantResponse) return undefined;
	try {
		return await config.beforeAssistantResponse({ assistantMessage, toolCalls, context: currentContext }, signal);
	} catch (error) {
		return {
			block: true,
			reason: `Assistant response validation failed: ${toolErrorText(error)}`,
			terminate: true,
		};
	}
}

async function rejectToolCallsBeforeExecution(
	toolCalls: AgentToolCall[],
	preflight: BeforeAssistantResponseResult,
	emit: AgentEventSink,
	...
	for (const toolCall of toolCalls) {
		const message: ToolResultMessage = {
			...
			content: [{ type: "text", text: preflight.reason || "Assistant response was rejected" }],
	...
	return { messages, terminate: preflight.terminate === true };
}

function createPreflightFailureMessage(source: AssistantMessage, errorMessage: string): AssistantMessage {
	return {
		role: "assistant",
		content: [],
		api: source.api,
		...
		model: source.model,
		...(source.responseModel ? { responseModel: source.responseModel } : {}),
		usage: {
			input: 0,
			output: 0,
			cacheRead: 0,
...
async function streamAssistantResponse(
	...
	if (!streamFn) {
		throw new Error("Agent loop requires a streamFn");
	}
