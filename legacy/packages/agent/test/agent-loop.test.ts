...
describe("assistant response preflight", () => {
	it("rejects a complete tool batch before any execution event or side effect", async () => {
		let executions = 0;
		...
		const config: AgentLoopConfig = {
			...
			beforeAssistantResponse: async () => {
				...
				return preflightCalls === 1
					? {
							block: true,
							reason: "declare an action first",
							details: { internal: true },
							responseMetadata: { disposition: "rejected" },
						}
					: undefined;
			},
		};
			...
			queueMicrotask(() => {
				const message =
					modelCalls++ === 0
						? createAssistantMessage(
								[
									{ type: "toolCall", id: "blocked-1", name: "echo", arguments: { value: "a" } },
									{ type: "toolCall", id: "blocked-2", name: "echo", arguments: { value: "b" } },
								],
								"toolUse",
							)
		...
		expect(
			events.find(
				(event) =>
					...
					event.responseMetadata !== undefined,
			),
		).toMatchObject({ responseMetadata: { disposition: "rejected" } });
		const results = messages.filter((message) => message.role === "toolResult");
		expect(results.map((message) => message.toolCallId)).toEqual(["blocked-1", "blocked-2"]);
		expect(results.every((message) => message.isError && message.details?.internal === true)).toBe(true);
	});

	it("uses a correction message to retry a rejected response without tool calls", async () => {
		const correction: AgentMessage = {
			role: "custom",
			customType: "protocol-correction",
			content: "include set_action",
			display: false,
			timestamp: Date.now(),
		};
		let preflightCalls = 0;
		const config: AgentLoopConfig = {
			...
			beforeAssistantResponse: async () =>
				preflightCalls++ === 0
					? { block: true, reason: "missing action", correctionMessage: correction }
					: undefined,
		};
		let modelCalls = 0;
		const stream = agentLoop(
			...
			() => {
				...
				queueMicrotask(() => {
					const message = createAssistantMessage([
						{ type: "text", text: modelCalls++ === 0 ? "stage only" : "corrected" },
					]);
		...
		const messages = await stream.result();
		expect(modelCalls).toBe(2);
		expect(messages.some((message) => message === correction)).toBe(true);
		expect(messages.at(-1)).toMatchObject({ role: "assistant", content: [{ text: "corrected" }] });
	});

	it("emits a terminal assistant error when a blocked response terminates the run", async () => {
		let executions = 0;
		const tool: AgentTool = {
			name: "echo",
			label: "Echo",
		...
		const config: AgentLoopConfig = {
			model: createModel(),
			convertToLlm: identityConverter,
			beforeAssistantResponse: async () => ({ block: true, reason: "correction limit exceeded", terminate: true }),
		};
		const stream = agentLoop(
			[createUserMessage("run")],
			...
			() => {
				...
				queueMicrotask(() => {
					const message = createAssistantMessage(
						[{ type: "toolCall", id: "blocked", name: "echo", arguments: {} }],
						"toolUse",
					);
		...
		expect(messages.at(-1)).toMatchObject({
			role: "assistant",
			stopReason: "error",
			errorMessage: "correction limit exceeded",
		});
	});
});
