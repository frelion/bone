...
3. **Read each lockstep package's [Unreleased] section:**
   - Derive the current package list from `LOCKSTEP_PACKAGE_DIRS` in `scripts/sync-versions.js`.
   - Read `packages/<name>/CHANGELOG.md` for every listed package.
...
6. **Add New Features section after changelog fixes:**
   ...
   - Select and write the top user-facing features from the audited commits without requiring separate confirmation during an explicitly requested release.
