...
const changelogs = LOCKSTEP_PACKAGE_DIRS.map((packageName) => `packages/${packageName}/CHANGELOG.md`);
const SEMVER_RE = /^\d+\.\d+\.\d+$/;
const releasePaths = new Set([
	"bun.lock",
	...LOCKSTEP_PACKAGE_DIRS.flatMap((packageName) => [
...
function assertReleaseChanges(paths) {
	const unexpected = paths.filter((path) => !releasePaths.has(path));
...
function publish(version) {
	...
	const nativeInputsChanged = Boolean(
		capture("git", [
			...
			"packages/memory/native/CMakeLists.txt",
			"packages/memory/native/bone-embed-addon.cpp",
