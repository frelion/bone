...
function buildStandalone() {
	...
	const packageDirectories = [
		...
		join(repoRoot, "packages", "session"),
		join(repoRoot, "packages", "protocol"),
		join(repoRoot, "packages", "images"),
		join(repoRoot, "packages", "forge"),
		join(repoRoot, "packages", "memory"),
		codingAgentDir,
	];
	...
	run(
		...
		[
			...
			"--compile",
			"./dist/bun/cli.js",
			"../images/src/image-resize-worker.ts",
			"../memory/src/local-embedding-worker.ts",
			"../memory/src/local-embedding-setup-worker.ts",
			"--outfile",
			"dist/bone",
		],
