...
First things first: **Bone's core is minimal**.

If your feature does not belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.

Bone's core exists to be minimal and extensible so it can be influenced and manipulated by extensions. Even extension hook points should be well considered to avoid unmaintainable bloat and complex interactions.
...
If you use an agent, run it from the Bone repository root so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
...
Bone receives more issues than the maintainers can responsibly review in real time. Many reports do not meet the quality bar in this guide or do not follow CONTRIBUTING.md. Some are submitted mindlessly via an agent instead of being reviewed and shaped by the person submitting them. Auto-closing creates a buffer so maintainers can review the tracker on their own schedule and reopen the issues that meet the quality bar.
...
Larger changes are discussed through repository issues and design documents.
